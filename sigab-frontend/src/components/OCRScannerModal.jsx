          ...
          <div className="flex-1 flex items-center justify-center min-h-[280px]">
            {!preview && mode === 'camera' && !cameraError && (
              <div className="relative w-full">
                <video
                  ref={videoRef}
                  className="w-full max-h-[55vh] rounded-lg bg-[var(--content-surface)] object-contain"
                  playsInline muted
                />
                {streaming && (
                  <button
                    onClick={captureFromCamera}
                    className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-white text-[var(--content-text)] w-16 h-16 rounded-full shadow-lg border-4 border-emerald-500 hover:scale-105 transition-transform flex items-center justify-center text-2xl"
                    aria-label="Capturar"
                  >
                    📸
                  </button>
            ...
            {!preview && mode === 'camera' && cameraError && (
              <div className="text-center text-[var(--content-muted)] p-6">
            ...
            {!preview && mode === 'file' && (
              <div
                className="w-full h-64 border-2 border-dashed border-[var(--content-border)] rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-emerald-500 hover:bg-[var(--content-surface)]/50 transition-all text-[var(--content-muted)] group"
                onClick={() => fileInputRef.current?.click()}
              >
                <span className="text-4xl mb-3 group-hover:scale-110 transition-transform">📄</span>
                <p className="font-medium text-[var(--content-muted)]">Click para seleccionar imagen</p>
              ...
              <div className="relative w-full flex flex-col items-center">
                ...
                {scanning && (
                  <div className="absolute inset-0 bg-emerald-500/10 backdrop-blur-[2px] flex items-center justify-center border-t-2 border-b-2 border-emerald-400 animate-[scan_2s_ease-in-out_infinite]">
                    <div className="flex bg-[var(--content-bg)]/90 text-emerald-600 px-4 py-2 rounded-full border border-emerald-500/50 shadow-emerald-500/20 shadow-lg items-center gap-2">
                ...
                <button
                  onClick={reintentar}
                  className="mt-3 px-4 py-2 bg-[var(--content-surface)] hover:bg-[var(--content-bg)] rounded-lg text-sm transition-colors text-[var(--content-text)]"
                >
                  ↺ Capturar otra
                </button>
              </div>
        ...
        <div className="md:w-1/2 p-4 flex flex-col bg-[var(--content-bg)] overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-2">
            {!file && !scanning && !scanResult && (
              <div className="text-center text-[var(--content-muted)] py-12">
                <div className="text-5xl mb-3">🤖</div>
                <p className="font-medium text-[var(--content-muted)]">Esperando captura…</p>
                <p className="text-xs mt-2">
                  Asegúrate que la hoja esté bien iluminada y el banner <code className="text-emerald-600">SIGAH-IMSS-OS-V3</code> sea visible.
                </p>
              </div>
            )}
            ...
            {scanning && (
              <div className="space-y-2 animate-pulse">
                {[1, 2, 3, 4, 5].map((i) => (
                  <div key={i} className="h-9 bg-[var(--content-surface)] rounded"></div>
                ))}
              </div>
            )}
              ...
              <>
                ...
                <div className="flex items-center justify-between bg-[var(--content-surface)] border border-[var(--content-border)]/50 rounded-lg px-3 py-2 mb-2">
                  <span className="text-xs text-[var(--content-muted)]">
                    Motor: <span className="text-emerald-600 font-mono">{scanResult.engine || 'gemma'}</span>
                  </span>
                  <span className="text-xs text-[var(--content-muted)]">
                    Confianza:{' '}
                    <span
                      ...
                      <div key={key}>
                        ...
                        {isLong ? (
                          <textarea
                            ...
                            className="w-full bg-[var(--content-surface)] border border-[var(--content-border)] text-emerald-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-500 h-20 resize-none"
                          />
                        ) : (
                          <input
                            type="text"
                            ...
                            className={`w-full bg-[var(--content-surface)] border rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-emerald-500 ${
                              val ? 'border-emerald-500/30 text-emerald-700' : 'border-[var(--content-border)] text-[var(--content-muted)]'
                            }`}
                          />
                        )}
                        {val == null && (
                          <p className="text-[10px] text-amber-600 mt-0.5">⚠ No detectado — completar manualmente</p>
                        )}
                      </div>
                    );
                  })}
                  ...
                  <div className="border-t border-[var(--content-border)] pt-2">
                    ...
                    <ul className="space-y-1 text-xs">
                      {scanResult.refacciones.map((r, i) => (
                        <li key={i} className="bg-[var(--content-surface)] border border-[var(--content-border)] rounded px-2 py-1 text-[var(--content-text)]">
          ...
          <div className="pt-3 border-t border-[var(--content-border)] mt-3 flex justify-end gap-2">
            <button
              onClick={onClose}
              className="px-3 py-2 bg-[var(--content-surface)] text-[var(--content-text)] rounded-lg hover:bg-[var(--content-bg)] transition text-sm"
            >
              Cancelar
            </button>
            <button
