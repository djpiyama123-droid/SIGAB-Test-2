...
const STATUS_CONFIG = {
  ...
  baja:             { border: '#475569', bg: 'var(--content-bg)', pulse: false, label: 'Baja' },
};

const CRITICIDAD_CONFIG = {
  alta:  { badge: 'bg-red-100 text-red-700 border border-red-300',   label: 'Alto Riesgo' },
  media: { badge: 'bg-yellow-100 text-yellow-700 border border-yellow-300', label: 'Riesgo Medio' },
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 pt-2.5 pb-1">
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          ...
          <span
            ...
            style={{
              backgroundColor: conFalla ? '#7f1d1d' : 'var(--content-bg)',
              color: conFalla ? '#fca5a5' : 'var(--content-muted)',
            }}
      ...
      {tieneEquipos && (
        <div className="flex items-center gap-2 px-3 pb-1.5 text-[9px] font-medium">
          {counts.operativo > 0 && (
            <span className="text-emerald-700">● {counts.operativo} OK</span>
          )}
          {counts.en_mantenimiento > 0 && (
            <span className="text-amber-600">● {counts.en_mantenimiento} Mant.</span>
          )}
...
function FichaTecnica({ equipo, onClose, onAbrirOS, onVerHistorial, onAbrirQR, onAccionRapida, onProgramarPreventivo }) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-50 w-80 shadow-2xl flex flex-col"
         style={{ backgroundColor: 'var(--content-bg)', borderLeft: '1px solid var(--content-border)' }}>
        ...
        <div className="mx-4 border-t border-[var(--content-border)] pt-3 pb-2 space-y-2.5">
          {equipo.clase_cofepris && (
            <div className="flex justify-between items-center">
              ...
              <span className="bg-purple-100 text-purple-700 border border-purple-300 text-xs px-2 py-0.5 rounded-full font-semibold">
        ...
        <div className="mx-4 border-t border-[var(--content-border)] pt-3 pb-2 space-y-2.5">
          ...
          {equipo.fecha_proximo_mantenimiento && (
            <div className="flex justify-between text-xs">
              ...
              <span className={mantenimientoVencido ? 'text-red-600 font-semibold' : 'text-[var(--content-muted)]'}>
                {new Date(equipo.fecha_proximo_mantenimiento).toLocaleDateString('es-MX')}
...
export default function HospitalMap() {
  ...
  if (error) {
    return (
      <div className="flex items-center gap-3 p-4 rounded-xl border border-red-300 bg-red-50 text-red-600 text-sm">
        <span>⚠</span>
        <span>{error}</span>
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <div className="relative flex-1 min-w-[160px]">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 bg-[var(--content-bg)]/60 border border-[var(--content-border)] rounded-lg text-xs text-[var(--content-text)] placeholder-[var(--content-muted)] focus:outline-none focus:border-blue-500"
              />
            </div>
        ...
        >
          {zonasFiltradas.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-40 text-[var(--content-muted)] gap-2">
              ...
              <button onClick={limpiarFiltros} className="text-xs text-blue-600 hover:text-blue-700 underline">
                Limpiar filtros
              </button>
            </div>
          ) : (
