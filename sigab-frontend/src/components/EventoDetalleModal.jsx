  ...
  if (loading || !data) {
    return (
      <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center">
        <div className="text-[var(--content-text)]">Cargando detalles...</div>
      </div>
    );
  }
        ...
        {TIMELINE_STEPS.map((st, i) => (
          <div key={st} className="flex items-center gap-1 flex-1">
            <div className={`flex-1 text-center px-1 py-1.5 text-[10px] font-semibold rounded-md whitespace-nowrap ${
              i <= idx ? 'bg-emerald-600/30 text-emerald-700 border border-emerald-500/40' :
              'bg-[var(--content-bg)] text-[var(--content-muted)]'
            }`}>
              {TV_ESTADO_LABELS[st]?.replace('Escalado COFEPRIS', 'COFEPRIS') || st}
            </div>
            {i < TIMELINE_STEPS.length - 1 && (
              <div className={`w-4 h-0.5 flex-shrink-0 ${i < idx ? 'bg-emerald-500' : 'bg-[var(--content-surface)]'}`} />
            )}
          </div>
        ))}
      </div>
        ...
        <div className="flex justify-between items-center p-4 border-b border-[var(--content-border)] sticky top-0 bg-[var(--content-surface)] z-10">
          <div className="flex items-center gap-3">
            <div>
              ...
              <p className="text-sm text-[var(--content-muted)]">{evento.dispositivo_nombre}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-[var(--content-bg)]/50 p-4 rounded-lg border border-[var(--content-border)]">
                <h3 className="text-sm font-semibold text-emerald-700 mb-2">Dispositivo medico (snapshot)</h3>
                <div className="grid grid-cols-2 gap-y-1.5 text-xs">
                  {[
                    ...
                    <div key={label}>
                      ...
                      <span className="text-[var(--content-muted)]">{val || 'N/A'}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Clasificacion */}
              <div className="bg-[var(--content-bg)]/50 p-4 rounded-lg border border-[var(--content-border)]">
                <h3 className="text-sm font-semibold text-blue-700 mb-2">Clasificacion</h3>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div>
                    <span className="text-[var(--content-muted)]">Fecha evento: </span>
                    <span className="text-[var(--content-muted)]">{formatFecha(evento.fecha_evento)}</span>
                  </div>
                  <div>
                    <span className="text-[var(--content-muted)]">Lugar: </span>
                    <span className="text-[var(--content-muted)]">{evento.lugar_evento || 'N/A'}</span>
                  </div>
                </div>
              </div>

              {/* Descripcion */}
              <div className="bg-[var(--content-bg)]/50 p-4 rounded-lg border border-[var(--content-border)] space-y-3">
                <div>
                  <h4 className="text-xs font-semibold text-[var(--content-muted)] mb-1">Descripcion del evento</h4>
                  <p className="text-sm text-[var(--content-muted)]">{evento.descripcion_evento}</p>
                </div>
                {evento.consecuencia_clinica && (
                  <div>
                    <h4 className="text-xs font-semibold text-[var(--content-muted)] mb-1">Consecuencia clinica</h4>
                    <p className="text-sm text-[var(--content-muted)]">{evento.consecuencia_clinica}</p>
                  </div>
                )}
                {evento.accion_correctiva && (
                  <div>
                    <h4 className="text-xs font-semibold text-[var(--content-muted)] mb-1">Accion correctiva</h4>
                    <p className="text-sm text-[var(--content-muted)]">{evento.accion_correctiva}</p>
                  </div>
                )}
              </div>
              ...
              {evento.paciente_sexo !== 'no_aplica' && (
                <div className="bg-[var(--content-bg)]/50 p-4 rounded-lg border border-[var(--content-border)]">
                  <h3 className="text-xs font-semibold text-yellow-700 mb-1">Paciente (anonimizado)</h3>
                  <div className="text-xs text-[var(--content-muted)]">
                    Sexo: <span className="text-[var(--content-muted)]">
                      {{ M: 'Masculino', F: 'Femenino', otro: 'Otro' }[evento.paciente_sexo] || 'N/A'}
                    </span>
                    {evento.paciente_edad && (
                      <> | Edad: <span className="text-[var(--content-muted)]">{evento.paciente_edad} anios</span></>
                    )}
                  </div>
                </div>
              )}
            ...
            <div className="space-y-4">
              {/* Investigacion */}
              <div className="bg-[var(--content-bg)]/50 p-4 rounded-lg border border-[var(--content-border)]">
                <h3 className="text-sm font-semibold text-purple-700 mb-2">Investigacion</h3>
                {evento.hallazgos || evento.causa_raiz ? (
                  <div className="space-y-2 text-xs">
                    {evento.investigador_nombre && (
                      <p className="text-[var(--content-muted)]">Investigador: <span className="text-[var(--content-muted)]">{evento.investigador_nombre}</span></p>
                    )}
                    {evento.fecha_investigacion && (
                      <p className="text-[var(--content-muted)]">Fecha: <span className="text-[var(--content-muted)]">{formatFecha(evento.fecha_investigacion)}</span></p>
                    )}
                    {evento.hallazgos && (
                      <div>
                        <h4 className="text-[var(--content-muted)] font-semibold">Hallazgos</h4>
                        <p className="text-[var(--content-muted)] mt-0.5">{evento.hallazgos}</p>
                      </div>
                    )}
                    {evento.causa_raiz && (
                      <div>
                        <h4 className="text-[var(--content-muted)] font-semibold">Causa raiz</h4>
                        <p className="text-[var(--content-muted)] mt-0.5">{evento.causa_raiz}</p>
                      </div>
                    )}
                  </div>
                ) : (
                  <p className="text-xs text-[var(--content-muted)]">Sin investigacion aun.</p>
                )}
              </div>
              ...
              {evento.enviado_cofepris && (
                <div className="bg-orange-50 p-4 rounded-lg border border-orange-200">
                  <h3 className="text-sm font-semibold text-orange-700 mb-2">Escalado COFEPRIS</h3>
                  <div className="text-xs text-[var(--content-muted)] space-y-1">
                    <p>Folio: <span className="text-orange-700 font-mono">{evento.folio_cofepris || 'Pendiente'}</span></p>
                    <p>Fecha envio: <span className="text-[var(--content-muted)]">{formatFecha(evento.fecha_envio_cofepris)}</span></p>
                  </div>
                </div>
              )}
              ...
              {evento.conclusion && (
                <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-200">
                  <h3 className="text-sm font-semibold text-emerald-700 mb-2">Conclusion y cierre</h3>
                  <p className="text-xs text-[var(--content-muted)]">{evento.conclusion}</p>
                  <p className="text-xs text-[var(--content-muted)] mt-1">Cerrado: {formatFecha(evento.fecha_cierre)}</p>
                </div>
              )}

              {/* Evidencias */}
              <div className="bg-[var(--content-bg)]/50 p-4 rounded-lg border border-[var(--content-border)]">
                <h3 className="text-sm font-semibold text-[var(--content-muted)] mb-2">Evidencias</h3>
                {evidencias.length === 0 ? (
                  <p className="text-xs text-[var(--content-muted)]">Sin evidencias aun</p>
                ) : (
                    ...
                    {evidencias.map((ev) => (
                      <div key={ev.id} className="flex items-center gap-2 text-xs bg-[var(--content-surface)] rounded p-2">
                        ...
                        <div className="flex-1">
                          <span className="text-[var(--content-muted)] uppercase tracking-wide text-[10px]">{ev.tipo}</span>
                          {ev.descripcion && <p className="text-[var(--content-muted)]">{ev.descripcion}</p>}
                          {ev.subido_por_nombre && <p className="text-[var(--content-muted)]">por {ev.subido_por_nombre}</p>}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {!isTerminal && (
                  <form onSubmit={handleSubirEvidencia} className="mt-3 space-y-2 border-t border-[var(--content-border)] pt-3">
                    <div className="flex gap-2">
                      <select value={evidTipo} onChange={(e) => setEvidTipo(e.target.value)}
                        className="bg-[var(--content-surface)] border border-[var(--content-border)] text-xs text-[var(--content-text)] p-1.5 rounded">
                      ...
                      <input type="text" placeholder="Descripcion..." value={evidDesc}
                        ...
                        className="flex-1 bg-[var(--content-surface)] border border-[var(--content-border)] text-xs text-[var(--content-text)] px-2 py-1.5 rounded" />
                    </div>
              ...
              {!isTerminal && (
                <div className="space-y-2">
                  ...
                  {/* Form investigacion */}
                  {showInvestigar && (
                    <div className="bg-[var(--content-bg)] border border-yellow-500/30 p-4 rounded-lg space-y-3">
                      ...
                      <textarea rows={3} value={formInv.hallazgos}
                        ...
                        className="w-full bg-[var(--content-surface)] border border-[var(--content-border)] text-xs text-[var(--content-text)] p-2 rounded" />
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => setShowInvestigar(false)}
                          className="flex-1 py-1.5 bg-[var(--content-surface)] border border-[var(--content-border)] text-[var(--content-text)] rounded text-sm hover:bg-[var(--content-border)]">Cancelar</button>
                  ...
                  {showEscalar && (
                    <div className="bg-[var(--content-bg)] border border-orange-500/30 p-4 rounded-lg space-y-3">
                      <h4 className="text-sm font-semibold text-orange-700">Escalar a COFEPRIS</h4>
                  ...
                  {showCerrar && (
                    <div className="bg-[var(--content-bg)] border border-emerald-500/30 p-4 rounded-lg space-y-3">
                      <h4 className="text-sm font-semibold text-emerald-700">Cerrar reporte</h4>
                  ...
                  {!isTerminal && !showInvestigar && !showEscalar && !showCerrar && (
                    <button onClick={handleCancelar}
                      className="w-full py-1.5 bg-[var(--content-surface)] hover:bg-[var(--content-border)] text-[var(--content-muted)] text-xs rounded-lg transition-colors">
                      Cancelar reporte
                    </button>
                  )}
                </div>
              ...
              {/* Metadata */}
              <div className="text-[10px] text-[var(--content-muted)] space-y-0.5">
