...
export default function FormatoViewer({ orden: initialOrden, onClose, autoprint = false }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/85 z-50 overflow-y-auto">
      ...
      {/* Barra de controles */}
      <div className="sticky top-0 bg-[var(--content-surface)] border-b border-[var(--content-border)] backdrop-blur-sm px-4 py-3 z-10">
        <div className="flex flex-wrap items-center gap-2 max-w-5xl mx-auto">
          <span className="text-[var(--content-text)] font-medium text-sm truncate max-w-xs">{titulo}</span>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded text-xs font-medium border transition-colors ${
                  tema === t.id
                    ...
                    : 'bg-[var(--content-surface)] border-[var(--content-border)] text-[var(--content-muted)] hover:bg-[var(--content-bg)]'
          ...
          <div className="flex gap-2 ml-auto">
              ...
              isEditing ? (
                <>
                  ...
                  <button
                    ...
                    className="px-4 py-1.5 bg-[var(--content-bg)] hover:bg-[var(--content-surface)] text-[var(--content-muted)] text-xs font-semibold rounded border border-[var(--content-border)] transition-colors"
                  >
                    ✕ Cancelar
                  </button>
                </>
            ...
            <button
              onClick={onClose}
              className="px-3 py-1.5 bg-[var(--content-bg)] hover:bg-[var(--content-surface)] text-[var(--content-muted)] text-xs font-medium rounded border border-[var(--content-border)] transition-colors"
            >
              ✕ Cerrar
            </button>
          </div>
