  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[var(--content-surface)] border border-[var(--content-border)] rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--content-border)] bg-[var(--content-bg)]">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div>
            ...
            <select required value={form.equipo_id} onChange={e => set('equipo_id', e.target.value)}
              className="w-full bg-[var(--content-surface)] border border-[var(--content-border)] rounded-xl px-4 py-2.5 text-[var(--content-text)] focus:outline-none focus:border-blue-500 transition-colors">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Área de destino *</label>
              <input required value={form.area_destino} onChange={e => set('area_destino', e.target.value)}
                className="w-full bg-[var(--content-surface)] border border-[var(--content-border)] rounded-xl px-4 py-2.5 text-[var(--content-text)] placeholder:text-[var(--content-muted)] focus:outline-none focus:border-blue-500 transition-colors"
                placeholder="Ej. UCI, Urgencias, Quirófano" />
            </div>
            <div>
              <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Piso</label>
          ...
          <div>
            <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Motivo</label>
            <input value={form.motivo} onChange={e => set('motivo', e.target.value)}
              className="w-full bg-[var(--content-surface)] border border-[var(--content-border)] rounded-xl px-4 py-2.5 text-[var(--content-text)] placeholder:text-[var(--content-muted)] focus:outline-none focus:border-blue-500 transition-colors"
              placeholder="Ej. Préstamo temporal, mantenimiento, reubicación" />
          </div>
          <div>
            <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Notas adicionales</label>
            <textarea rows={2} value={form.notas} onChange={e => set('notas', e.target.value)}
              className="w-full bg-[var(--content-surface)] border border-[var(--content-border)] rounded-xl px-4 py-2.5 text-[var(--content-text)] placeholder:text-[var(--content-muted)] focus:outline-none focus:border-blue-500 transition-colors resize-none"
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      ...
      {loading ? (
        ...
        <div className="py-16 text-center bg-[var(--content-surface)] border-2 border-dashed border-[var(--content-border)] rounded-3xl">
          <MapPin className="h-10 w-10 text-[var(--content-muted)] mx-auto mb-3" />
          <p className="text-[var(--content-muted)] font-medium">Sin movimientos registrados.</p>
            ...
            <div key={m.id || i} className="flex gap-4">
              ...
              <div className="bg-[var(--content-surface)] border border-[var(--content-border)] rounded-lg p-4 flex-1 mb-1">
                ...
                <div className="flex items-center gap-2 mt-2 text-sm">
                  ...
                  <ArrowRight className="h-3 w-3 text-[var(--content-muted)]" />
