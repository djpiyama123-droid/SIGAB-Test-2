      ...
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-lg shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-slate-400 font-medium uppercase mb-1 block">Área de destino *</label>
              <input required value={form.area_destino} onChange={e => set('area_destino', e.target.value)}
                className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500 transition-colors"
                placeholder="Ej. UCI, Urgencias, Quirófano" />
            </div>
            <div>
              <label className="text-xs text-slate-400 font-medium uppercase mb-1 block">Piso</label>
          ...
          <div>
            <label className="text-xs text-slate-400 font-medium uppercase mb-1 block">Motivo</label>
            <input value={form.motivo} onChange={e => set('motivo', e.target.value)}
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500 transition-colors"
              placeholder="Ej. Prestamo temporal, mantenimiento, reubicación" />
          </div>
          <div>
            <label className="text-xs text-slate-400 font-medium uppercase mb-1 block">Notas adicionales</label>
            <textarea rows={2} value={form.notas} onChange={e => set('notas', e.target.value)}
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500 transition-colors resize-none"
    ...
    <div className="p-4 md:p-6 space-y-5">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-slate-400 text-sm">Historial de movimientos y traslados — NOM-016</p>
        </div>
        <button
              ...
              <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 flex-1 mb-1">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <span className="text-slate-500 text-xs ml-2 font-mono">{m.equipo_serie}</span>
                  </div>
                ...
                {m.motivo && (
                  <p className="text-slate-500 text-xs mt-1.5">Motivo: {m.motivo}</p>
                )}
              </div>
            </div>
          ))}
