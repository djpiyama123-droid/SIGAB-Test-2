    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[var(--content-surface)] border border-[var(--content-border)] rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Tema de capacitación *</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Fecha *</label>
              <input type="date" required value={form.fecha_capacitacion} onChange={e => set('fecha_capacitacion', e.target.value)}
                className="w-full bg-[var(--content-bg)] border border-[var(--content-border)] rounded-xl px-4 py-2.5 text-[var(--content-text)] focus:outline-none focus:border-emerald-500 transition-colors" />
            </div>
            <div>
              <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Instructor</label>
          ...
          <div>
            <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Personal capacitado *</label>
            <textarea required rows={3} value={form.personal_capacitado} onChange={e => set('personal_capacitado', e.target.value)}
              className="w-full bg-[var(--content-bg)] border border-[var(--content-border)] rounded-xl px-4 py-2.5 text-[var(--content-text)] placeholder:text-[var(--content-muted)] focus:outline-none focus:border-emerald-500 transition-colors resize-none"
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {capacitaciones.map((item) => (
            <div
              key={item.id}
              className="bg-[var(--content-surface)] border border-[var(--content-border)] hover:border-emerald-500/50 transition-all p-6 rounded-3xl group cursor-default">
              ...
              <h3 className="text-lg font-bold text-[var(--content-text)] mb-1 group-hover:text-emerald-600 transition-colors">
                {item.tema}
              </h3>
