...
const PRIORIDAD_STYLE = {
  critica: { bar: 'bg-red-500',    badge: 'bg-red-100 text-red-700',    icon: '🚨' },
  alta:    { bar: 'bg-orange-500', badge: 'bg-orange-100 text-orange-700', icon: '⚠️' },
  media:   { bar: 'bg-yellow-500', badge: 'bg-yellow-100 text-yellow-700', icon: '📋' },
        ...
        {['', 'critica', 'alta', 'media', 'baja'].map((p) => (
          <button key={p} onClick={() => setFiltro(p)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors ${
              filtro === p ? 'bg-emerald-100 text-emerald-700' : 'bg-[var(--content-surface)] text-[var(--content-muted)] hover:bg-[var(--content-border)]'
            }`}>
            {p || 'Todas'}
          </button>
        ))}
