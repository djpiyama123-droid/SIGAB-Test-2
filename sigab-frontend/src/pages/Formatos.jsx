...
const COLOR_CLASSES = {
  amber:   { card: 'border-amber-700/50 hover:border-amber-500', badge: 'bg-amber-900/40 text-amber-300', btn: 'bg-amber-600 hover:bg-amber-700' },
  red:     { card: 'border-red-700/50 hover:border-red-500',     badge: 'bg-red-900/40 text-red-300',     btn: 'bg-red-600 hover:bg-red-700' },
  emerald: { card: 'border-emerald-700/50 hover:border-emerald-500', badge: 'bg-emerald-900/40 text-emerald-300', btn: 'bg-emerald-600 hover:bg-emerald-700' },
  purple:  { card: 'border-purple-700/50 hover:border-purple-500', badge: 'bg-purple-900/40 text-purple-300', btn: 'bg-purple-600 hover:bg-purple-700' },
};
  ...
  return (
    <div className="p-4 md:p-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white">Formatos Oficiales IMSS</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {FORMATOS_INFO.map((f) => {
          ...
          return (
            <div key={f.tipo}
              className={`bg-slate-800 border rounded-xl p-5 transition-colors ${c.card}`}>
              {/* Header de la card */}
              <div className="flex items-start gap-3 mb-3">
                <span className="text-2xl">{f.icon}</span>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h2 className="text-base font-bold text-white">{f.label}</h2>
              ...
              {/* Campos del formato */}
              <div className="mb-4">
                <p className="text-xs text-slate-500 uppercase font-medium mb-2">Secciones</p>
                <div className="flex flex-wrap gap-1.5">
                  {f.campos.map((campo) => (
                    <span key={campo}
                      className="px-2 py-0.5 bg-slate-700/60 text-slate-400 text-xs rounded border border-slate-600/50">
                      {campo}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs text-slate-500">{f.norma}</span>
                <button
