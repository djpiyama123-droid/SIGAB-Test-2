...
const SUSCRIPCION_BADGE = {
  activo: { dot: 'bg-emerald-500', pill: 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/25', label: 'Activo' },
  trial: { dot: 'bg-amber-500', pill: 'bg-amber-500/15 text-amber-400 border border-amber-500/25', label: 'Trial' },
  vencido: { dot: 'bg-red-500', pill: 'bg-red-500/15 text-red-400 border border-red-500/25', label: 'Vencido' },
  inactivo: { dot: 'bg-slate-500', pill: 'bg-slate-500/15 text-slate-400 border border-slate-500/25', label: 'Inactivo' },
};
  ...
  return (
    <GlassCard>
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="p-2.5 rounded-xl" style={{ backgroundColor: 'rgba(0,108,183,0.12)' }}>
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <section aria-label="KPIs globales">
        <h2 className="text-sm font-semibold uppercase tracking-widest text-[var(--content-muted)] mb-3">Resumen global</h2>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <KPICard icon={Hospital} label="Total Hospitales" value={stats?.total_hospitales ?? '—'} colorClass="text-blue-400" />
          <KPICard icon={Building2} label="Hospitales Activos" value={stats?.hospitales_activos ?? '—'} colorClass="text-emerald-400" />
          <KPICard icon={DollarSign} label="MRR" value={stats?.mrr_mxn != null ? `$${Number(stats.mrr_mxn).toLocaleString('es-MX')} MXN` : '—'} colorClass="text-amber-400" />
          <KPICard icon={Users} label="Usuarios Totales" value={stats?.total_usuarios ?? stats?.usuarios_totales ?? '—'} colorClass="text-slate-300" />
        </div>
      </section>

      <section aria-label="Hospitales registrados">
        <h2 className="text-sm font-semibold uppercase tracking-widest text-[var(--content-muted)] mb-3">Hospitales / Tenants</h2>
        <TableWrapper>
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: '1px solid var(--content-border)' }}>
                {['Hospital', 'Slug', 'Suscripción', 'Usuarios', 'Activo Desde', 'Acciones'].map((col) => (
                  <th key={col} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[var(--content-muted)]">{col}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center text-[var(--content-muted)] text-sm">Cargando...</td>
                </tr>
              ) : hospitales.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center text-[var(--content-muted)] text-sm">Sin hospitales registrados.</td>
                </tr>
              ) : (
                hospitales.map((h) => (
                  <tr key={h.id} style={{ borderBottom: '1px solid var(--content-border)' }} className="transition-colors hover:bg-white/[0.03]">
                    <td className="px-4 py-3.5 font-medium text-[var(--content-text)]">{h.nombre}</td>
                    <td className="px-4 py-3.5">
                      <code className="text-xs font-mono px-1.5 py-0.5 rounded bg-slate-500/15 text-slate-300">{h.slug}</code>
                    </td>
                    <td className="px-4 py-3.5"><SuscripcionBadge estado={h.estado_suscripcion} /></td>
                    <td className="px-4 py-3.5 text-[var(--content-muted)]">{h.num_usuarios ?? h.total_usuarios ?? '—'}</td>
                    <td className="px-4 py-3.5 text-[var(--content-muted)]">
                      {h.activo_desde ? new Date(h.activo_desde).toLocaleDateString('es-MX', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}
                    </td>
                    <td className="px-4 py-3.5">
                      <button disabled title="Próximamente — Fase 3"
      ...
      <section aria-label="Actividad reciente global">
        <h2 className="text-sm font-semibold uppercase tracking-widest text-[var(--content-muted)] mb-3">Actividad reciente</h2>
