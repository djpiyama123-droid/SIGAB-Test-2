...
const PRIORIDAD_BADGE = {
  critica: 'bg-red-100 text-red-700 border border-red-300',
...
const ESTADO_BADGE = {
  abierta:     'bg-red-100 text-red-600',
    ...
    <div className="p-4 md:p-6 space-y-5">
                ...
                {archivos.map((a) => (
                  <a key={a.nombre} href={a.url} target="_blank" rel="noopener noreferrer"
                    ...
                    <div className="flex justify-between items-start">
                      <span className="font-mono text-xs text-emerald-700">{a.folio}</span>
                    ...
                    <tbody>
                      {archivos.map((a) => (
                        <tr key={a.nombre} className="border-t border-[var(--content-border)]/50 hover:bg-[var(--content-border)]/40 transition-colors">
                          <td className="px-4 py-2 font-mono text-xs text-emerald-700">{a.folio}</td>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {FILTROS_ESTADO.map((e) => (
          <button key={e} onClick={() => setEstado(e)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors ${
              estadoFiltro === e ? 'bg-emerald-100 text-emerald-700' : 'bg-[var(--content-surface)] text-[var(--content-muted)] hover:bg-[var(--content-border)]'
            }`}>
            {e || 'Todas'}
          </button>
        ))}
        ...
        {FILTROS_TIPO.map((t) => (
          <button key={t} onClick={() => setTipo(t)}
            className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize transition-colors ${
              tipoFiltro === t ? 'bg-blue-100 text-blue-700' : 'bg-[var(--content-surface)] text-[var(--content-muted)] hover:bg-[var(--content-border)]'
            }`}>
            {t || 'Todos'}
          </button>
        ))}
      ...
      {loading ? (
        ...
        <div className="bg-[var(--content-surface)] rounded-xl border border-[var(--content-border)] overflow-hidden">
          {/* Vista móvil: Cards */}
          <div className="block sm:hidden divide-y divide-[var(--content-border)]">
            {ordenes.map((os) => (
              <div
                key={os.id}
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <span className="font-mono text-xs text-emerald-700">{os.numero_orden}</span>
                ...
                <div className="flex justify-between items-center mt-4 pt-3 border-t border-[var(--content-border)]/30">
                  ...
                  <div className="flex gap-3" onClick={(e) => e.stopPropagation()}>
                    <button
                      ...
                      className="px-3 py-1 bg-teal-50 text-teal-700 rounded-lg border border-teal-200 text-xs"
                    >
                      📋 CENEVAL
                    </button>
                    {os.estado !== 'cerrada' && os.estado !== 'cancelada' && (
                      <button onClick={() => handleCerrar(os.id)}
                        className="px-3 py-1 bg-emerald-50 text-emerald-700 rounded-lg border border-emerald-200 text-xs">
                        Cerrar
                      </button>
                    )}
                  </div>
                  ...
                  <tr key={os.id}
                    ...
                    <td className="px-4 py-3 whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                      <div className="flex gap-2 flex-wrap">
                        {os.estado !== 'cerrada' && os.estado !== 'cancelada' && (
                          <button onClick={() => handleCerrar(os.id)}
                            className="text-xs text-emerald-700 hover:text-emerald-600 hover:underline">
                            Cerrar
                          </button>
                        )}
                        <button
                        ...
                        <button
                          ...
                          className="text-xs text-teal-700 hover:text-teal-600 hover:underline"
                        >
                          📋
                        </button>
                        <button
                          ...
                          className="text-xs text-purple-700 hover:text-purple-600 hover:underline"
