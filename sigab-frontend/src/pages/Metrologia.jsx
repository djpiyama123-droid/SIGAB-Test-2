        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="text-xs text-slate-400 font-medium uppercase mb-1 block">Tipo de medición *</label>
            ...
            <div>
              <label className="text-xs text-slate-400 font-medium uppercase mb-1 block">Fecha de calibración *</label>
              <input type="date" required value={form.fecha_calibracion} onChange={e => set('fecha_calibracion', e.target.value)}
                className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-blue-500 transition-colors" />
            </div>
            <div>
              <label className="text-xs text-slate-400 font-medium uppercase mb-1 block">Vigencia (meses)</label>
              <input type="number" min={1} max={120} value={form.vigencia_meses} onChange={e => set('vigencia_meses', e.target.value)}
                className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-blue-500 transition-colors" />
            </div>
            <div>
              <label className="text-xs text-slate-400 font-medium uppercase mb-1 block">Entidad calibradora</label>
              <input value={form.entidad_calibradora} onChange={e => set('entidad_calibradora', e.target.value)}
                className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-2.5 text-white placeholder:text-slate-600 focus:outline-none focus:border-blue-500 transition-colors"
                placeholder="Laboratorio NOM-028" />
            </div>
            <div>
              <label className="text-xs text-slate-400 font-medium uppercase mb-1 block">Número de certificado</label>
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-800">
            {loading ? (
              <tr><td colSpan={6} className="px-6 py-12 text-center text-slate-500">Cargando calibraciones...</td></tr>
            ) : calibraciones.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-6 py-16 text-center">
                  <ShieldCheck className="h-10 w-10 text-slate-700 mx-auto mb-3" />
                  <p className="text-slate-500">Sin calibraciones registradas.</p>
                </td>
              </tr>
            ) : (
              calibraciones.map((c) => {
