    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-[var(--content-surface)] border border-[var(--content-border)] rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div>
            <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Equipo *</label>
            <select required value={form.equipo_id} onChange={e => set('equipo_id', e.target.value)}
              className="w-full bg-[var(--content-bg)] border border-[var(--content-border)] rounded-xl px-4 py-2.5 text-[var(--content-text)] focus:outline-none focus:border-blue-500 transition-colors">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Tipo de medición *</label>
              <input required value={form.tipo_medicion} onChange={e => set('tipo_medicion', e.target.value)}
                className="w-full bg-[var(--content-bg)] border border-[var(--content-border)] rounded-xl px-4 py-2.5 text-[var(--content-text)] placeholder:text-[var(--content-muted)] focus:outline-none focus:border-blue-500 transition-colors"
                placeholder="Ej. Presión arterial, Temperatura, Flujo O₂" />
            </div>
            <div>
              <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Fecha de calibración *</label>
              <input type="date" required value={form.fecha_calibracion} onChange={e => set('fecha_calibracion', e.target.value)}
                className="w-full bg-[var(--content-bg)] border border-[var(--content-border)] rounded-xl px-4 py-2.5 text-[var(--content-text)] focus:outline-none focus:border-blue-500 transition-colors" />
            </div>
            <div>
              <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Vigencia (meses)</label>
              <input type="number" min={1} max={120} value={form.vigencia_meses} onChange={e => set('vigencia_meses', e.target.value)}
                className="w-full bg-[var(--content-bg)] border border-[var(--content-border)] rounded-xl px-4 py-2.5 text-[var(--content-text)] focus:outline-none focus:border-blue-500 transition-colors" />
            </div>
            <div>
              <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Entidad calibradora</label>
              <input value={form.entidad_calibradora} onChange={e => set('entidad_calibradora', e.target.value)}
                className="w-full bg-[var(--content-bg)] border border-[var(--content-border)] rounded-xl px-4 py-2.5 text-[var(--content-text)] placeholder:text-[var(--content-muted)] focus:outline-none focus:border-blue-500 transition-colors"
                placeholder="Laboratorio NOM-028" />
            </div>
            <div>
              <label className="text-xs text-[var(--content-muted)] font-medium uppercase mb-1.5 block">Número de certificado</label>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose}
              className="flex-1 py-2.5 rounded-xl border border-[var(--content-border)] text-[var(--content-muted)] hover:bg-[var(--content-bg)] transition-all">
              Cancelar
            </button>
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-[var(--content-surface)] border border-[var(--content-border)] p-6 rounded-2xl">
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-[var(--content-border)]">
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-6 py-16 text-center bg-[var(--content-bg)] rounded-2xl">
                  <ShieldCheck className="h-10 w-10 text-[var(--content-muted)] mx-auto mb-3" />
                  <p className="text-[var(--content-muted)] font-medium">Sin calibraciones registradas.</p>
                </td>
              </tr>
            ) : (
              calibraciones.map((c) => {
                ...
                return (
                  <tr key={c.id} className="hover:bg-[var(--content-surface)] transition-colors group">
                    <td className="px-6 py-4">
                      <p className="font-bold text-[var(--content-text)] group-hover:text-blue-600 transition-colors">{c.equipo_nombre}</p>
                    ...
                    <td className="px-6 py-4 text-right">
                      <button
                        ...
                        className="text-blue-600 hover:text-blue-700 shadow-sm">
