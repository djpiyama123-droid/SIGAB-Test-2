...
export default function QRScanner() {
  ...
  return (
    <div className="min-h-screen bg-[var(--content-bg)] flex flex-col">
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-3 bg-[var(--content-surface)] border-b border-[var(--content-border)]">
        <button
          onClick={() => { stopCamera(); navigate(-1); }}
          className="text-[var(--content-muted)] hover:text-[var(--content-text)] p-1.5 rounded-lg hover:bg-[var(--content-bg)] transition-colors"
        >
        ...
        <div>
          <h1 className="text-[var(--content-text)] font-semibold text-base">Escanear QR de Equipo</h1>
          <p className="text-[var(--content-muted)] text-xs">Sistema SIGAH — HGR No.1 IMSS</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-start pt-6 px-4 gap-5">
        ...
        <div className="relative w-full max-w-sm">
          <div className="relative aspect-square rounded-2xl overflow-hidden bg-[var(--content-surface)] border-2 border-[var(--content-border)]">
            <video
              ref={videoRef}
            ...
            {/* Error de cámara */}
            {error && (
              <div className="absolute inset-0 bg-[var(--content-surface)]/95 flex flex-col items-center justify-center gap-3 p-6">
                <div className="w-14 h-14 rounded-full bg-red-100 flex items-center justify-center">
                  <svg className="w-7 h-7 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z" />
                  </svg>
                </div>
                <p className="text-[var(--content-muted)] text-sm text-center">{error}</p>
        ...
        {/* Instrucción */}
        <p className="text-[var(--content-muted)] text-sm text-center max-w-xs">
        ...
        <div className="flex items-center gap-3 w-full max-w-sm">
          <div className="flex-1 h-px bg-[var(--content-border)]" />
          <span className="text-[var(--content-muted)] text-xs">o ingresa manualmente</span>
          <div className="flex-1 h-px bg-[var(--content-border)]" />
        </div>
        ...
        <form onSubmit={handleManual} className="flex gap-2 w-full max-w-sm">
          <input
            ...
            className="flex-1 bg-[var(--content-surface)] border border-[var(--content-border)] rounded-lg px-3 py-2.5 text-sm text-[var(--content-text)] focus:outline-none focus:border-emerald-600 placeholder-[var(--content-muted)]"
          />
          <button type="submit"
        ...
        <p className="text-[var(--content-muted)] text-xs text-center max-w-xs pb-6">
