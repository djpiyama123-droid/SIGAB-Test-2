  ...
  if (isUser) {
    return (
      <div className="flex justify-end mb-4">
        <div className="max-w-[75%] bg-emerald-600 border border-emerald-600 rounded-2xl rounded-tr-sm px-4 py-2.5 text-sm text-white">
          {msg.content}
        </div>
      </div>
    );
    ...
    <div className="flex gap-3 mb-4">
      ...
      <div className={`max-w-[80%] rounded-2xl rounded-tl-sm px-4 py-2.5 text-sm ${
        isError
          ...
          : 'bg-[var(--content-bg)] border border-[var(--content-border)]/40 text-[var(--content-text)]'
      }`}>
        {isError ? (
          ...
          <div className="prose prose-sm max-w-none">
            {msg.content.split('\n').map((line, i) => {
              if (line.startsWith('**') && line.endsWith('**')) {
                return <p key={i} className="font-bold text-emerald-600 mt-2 mb-1">{line.replace(/\*\*/g, '')}</p>;
              }
  ...
  return (
    <div className="bg-[var(--content-surface)] border border-[var(--content-border)] rounded-xl p-4 space-y-3">
      <div className="flex justify-between items-center">
        <h3 className="text-sm font-semibold text-amber-600">Diagnóstico de Falla con IA</h3>
      ...
      {resultado && (
        <div className="bg-[var(--content-bg)]/60 border border-yellow-500/20 rounded-lg p-3 text-xs text-[var(--content-muted)] space-y-1 max-h-60 overflow-y-auto">
          {resultado.split('\n').map((line, i) => {
            if (line.startsWith('**') && line.endsWith('**'))
              return <p key={i} className="font-bold text-amber-600 mt-2">{line.replace(/\*\*/g, '')}</p>;
            if (line.trim() === '') return <br key={i} />;
...
function VisionPanel({ onClose }) {
  ...
  return (
    <div className="bg-[var(--content-surface)] border border-[var(--content-border)] rounded-xl p-4 space-y-3">
      <div className="flex justify-between items-center">
        <h3 className="text-sm font-semibold text-blue-600">Análisis de Imagen (Gemma Vision)</h3>
        <button onClick={onClose} className="text-[var(--content-muted)] hover:text-[var(--content-text)] text-xs">✕ Cerrar</button>
      </div>
      ...
      <div className="border-2 border-dashed border-[var(--content-border)] rounded-lg p-4 text-center">
        {imagen ? (
          <div className="space-y-2">
            ...
            <button onClick={() => setImagen(null)} className="text-xs text-red-600 hover:text-red-700">Cambiar</button>
          </div>
        ) : (
      ...
      {resultado && (
        <div className="bg-[var(--content-bg)]/60 border border-blue-500/20 rounded-lg p-3 text-xs text-[var(--content-muted)] space-y-2 max-h-60 overflow-y-auto">
          {resultado.datos_extraidos && (
            <div className="mb-2">
              <p className="text-blue-600 font-semibold mb-1">Datos extraídos:</p>
    ...
    <div className="h-full flex flex-col" style={{ minHeight: '0' }}>
      ...
      {ollamaStatus && !ollamaStatus.ollama_activo && (
        <div className="mb-4 bg-orange-50 border border-orange-300 rounded-xl p-4 text-sm flex-shrink-0">
          ...
          <code className="block mt-2 bg-orange-100 rounded p-2 text-xs text-orange-700 font-mono">
        ...
        `}>
          ...
          <div className="p-4 bg-[var(--content-surface)] border border-[var(--content-border)] rounded-xl space-y-2">
            ...
            <button onClick={generarResumenIA} disabled={cargandoResumen}
              className="w-full py-1.5 bg-emerald-600 hover:bg-emerald-500 border border-emerald-600 text-white text-xs font-semibold rounded-lg transition-colors disabled:opacity-50">
            ...
            {resumenIA && (
              <div className="mt-2 bg-[var(--content-bg)]/60 rounded-lg p-3 text-xs text-[var(--content-muted)] max-h-48 overflow-y-auto">
                <p className="text-emerald-600 font-semibold mb-1 text-[10px]">
          ...
          {ollamaStatus?.ollama_activo && (
            <div className="p-4 border border-[var(--content-border)] rounded-xl" style={{ background: 'var(--content-surface)' }}>
              ...
              <div className="space-y-1 text-[10px] text-[var(--content-muted)]">
                ...
                <p>Modo: <span className="text-emerald-600">100% on-premise</span></p>
                <p>Datos: <span className="text-emerald-600">No salen del servidor</span></p>
              </div>
              {ollamaStatus.modelos_instalados?.length > 0 && (
                <div className="mt-2">
