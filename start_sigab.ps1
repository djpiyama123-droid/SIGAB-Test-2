...
$BACKEND_DIR = Join-Path $SCRIPT_DIR "sigab-backend"
$FRONTEND_DIR = Join-Path $SCRIPT_DIR "sigab-frontend"
$BOT_DIR = Join-Path $SCRIPT_DIR "sigab-bot"
...
if ($RESET_DB) {
    ...
    cmd /c "mysql -u $DB_USER -p$DB_PASS $DB_NAME < $SCRIPT_DIR\sigab_schema.sql"
    cmd /c "mysql -u $DB_USER -p$DB_PASS $DB_NAME < $SCRIPT_DIR\seed_data.sql"
