...
SSH_KEY          = os.environ.get("SSH_KEY",          "")   # ruta a clave privada sin passphrase
...
def ssh_run(cmd: str, timeout=12) -> tuple[bool, str]:
    ...
    try:
        ssh_cmd = ["ssh", "-o", "ConnectTimeout=8", "-o", "BatchMode=yes",
                   "-o", "StrictHostKeyChecking=accept-new"]
        key = SSH_KEY.strip()
        if key:
            ssh_cmd += ["-i", os.path.expanduser(key)]
        ssh_cmd += [SSH_ALIAS, cmd]
...
def collect_graph() -> dict:
    """Mapa estático del ecosistema SIGAB — nodos y enlaces para el knowledge graph."""
    nodes = [
        # MACHINES
        {"id": "asus-tuf",    "label": "Asus TUF\nDev Local",           "group": "machine", "size": 20, "desc": "Máquina de desarrollo principal — Claude Code corre aquí"},
        {"id": "lenovo",      "label": "Lenovo\nOn-Premise",             "group": "machine", "size": 18, "desc": "Servidor on-premise Hospital IMSS Tijuana"},
        {"id": "vps",         "label": "VPS Bluehost\n129.121.100.147",  "group": "machine", "size": 22, "desc": "VPS público — backend + bot WhatsApp en producción"},
        # AI
        {"id": "claude-code", "label": "Claude Code\nIA Dev",            "group": "ai",      "size": 30, "desc": "Claude Sonnet 4.6 — agente de desarrollo autónomo"},
        {"id": "ollama",      "label": "Ollama :11434\nIA Local",        "group": "ai",      "size": 22, "desc": "Runtime de modelos LLM local en VPS (puerto cerrado externamente)"},
        {"id": "gemma",       "label": "Gemma 3:4B\nModelo",             "group": "ai",      "size": 16, "desc": "Modelo IA principal para copilot biomédico"},
        {"id": "qwen",        "label": "Qwen 2.5:7B\nModelo",            "group": "ai",      "size": 14, "desc": "Modelo IA alternativo en Ollama"},
        # INFRA
        {"id": "docker",      "label": "Docker Compose\nOrquestador",    "group": "infra",   "size": 20, "desc": "docker-compose.yml y docker-compose.hetzner.yml"},
        {"id": "nginx",       "label": "Nginx\nProxy Reverso",           "group": "infra",   "size": 16, "desc": "Reverse proxy — enruta /api → backend, / → frontend"},
        {"id": "mysql",       "label": "MySQL 8.0\n:3306",               "group": "db",      "size": 24, "desc": "Base de datos principal — 20+ tablas, multi-tenant"},
        # BACKEND
        {"id": "fastapi",     "label": "FastAPI\nBackend :8000",         "group": "backend", "size": 32, "desc": "sigab-backend/ — 102 endpoints, 20 routers"},
        {"id": "routes",      "label": "20 Routers\n/api/*",             "group": "backend", "size": 20, "desc": "routes/ — equipos, ordenes, copilot, tecnovigilancia, formatos…"},
        {"id": "services",    "label": "18 Services\nBusiness Logic",    "group": "backend", "size": 18, "desc": "services/ — gemma_service, ocr_service, reliability_service…"},
        {"id": "auth",        "label": "Auth JWT\n+ Multi-Tenant",       "group": "backend", "size": 18, "desc": "auth/ — JWT, bcrypt, tenancy isolation, bot_auth"},
        {"id": "copilot-api", "label": "Copilot\n/api/copilot",          "group": "backend", "size": 16, "desc": "routes/copilot.py — chat SSE, diagnostico, causa-raiz, vision"},
        {"id": "gemma-svc",   "label": "Gemma Service\nIA proxy",        "group": "backend", "size": 14, "desc": "services/gemma_service.py — _resolve_model(), chat_stream(), analizar_no_stream()"},
        # FRONTEND
        {"id": "react",       "label": "React 19\nFrontend :5173",       "group": "frontend", "size": 30, "desc": "sigab-frontend/ — Vite + Tailwind + Sileo"},
        {"id": "pages",       "label": "26 Pages\nUI Modules",           "group": "frontend", "size": 20, "desc": "pages/ — Dashboard, Equipos, Copilot, Tecnovigilancia, Formatos…"},
        {"id": "components",  "label": "50+ Components\nUI Library",     "group": "frontend", "size": 18, "desc": "components/ — EquipoCard, DashboardCharts, GlassCard, Toast…"},
        # CONFIG / MEMORY
        {"id": "claude-md",   "label": "CLAUDE.md\nProject Config",      "group": "config",  "size": 16, "desc": "Instrucciones de proyecto — stack, convenciones, módulos implementados"},
        {"id": "memory",      "label": "Memory Files\nAuto-Memoria",     "group": "config",  "size": 14, "desc": "~/.claude/projects/*/memory/ — perfil, estado, feedback, handoff"},
        {"id": "worktrees",   "label": "Git Worktrees\nBranches",        "group": "config",  "size": 14, "desc": ".claude/worktrees/ — ramas aisladas por tarea (fix-domain-sigab, etc.)"},
        {"id": "monitor",     "label": "SIGAB Monitor\n:4001",           "group": "config",  "size": 18, "desc": "sigab-monitor/ — este dashboard, server.py + dashboard.html"},
        # NUEVOS — del Workbook Maestro
        {"id": "hetzner",     "label": "Hetzner CX32\nStaging SaaS",    "group": "machine", "size": 18, "desc": "Servidor cloud futuro — SIGAH SaaS multi-tenant (por crear, ~$150 MXN/mes)"},
        {"id": "skills-proj", "label": "12 Skills\nProyecto",            "group": "config",  "size": 16, "desc": ".claude/skills/ — ui-ux-pro-max, mysql, ccpm, git-workflow, react-patterns, webapp-testing…"},
        {"id": "agents",      "label": "67 Agentes\nThe Agency",         "group": "config",  "size": 18, "desc": ".claude/agents/ — Backend Architect, Security Engineer, Code Reviewer, Technical Writer…"},
        {"id": "docs",        "label": "Docs SIGAH\n12 docs",            "group": "config",  "size": 14, "desc": "docs/SIGAH/ — Workbook Maestro, Plan de Trabajo, Propuesta Fiscal, Runbook Hetzner…"},
        {"id": "sigab-bot",   "label": "Bot WhatsApp\nNode.js :3000",    "group": "infra",   "size": 16, "desc": "sigab-bot/ — Bot WA para notificaciones IMSS, enlazado con OpenClaw/bot-auth"},
    ]
    links = [
        {"source": "asus-tuf",    "target": "claude-code", "rel": "runs"},
        {"source": "asus-tuf",    "target": "fastapi",     "rel": "dev"},
        {"source": "asus-tuf",    "target": "react",       "rel": "dev"},
        {"source": "asus-tuf",    "target": "monitor",     "rel": "runs"},
        {"source": "lenovo",      "target": "fastapi",     "rel": "hosts"},
        {"source": "lenovo",      "target": "mysql",       "rel": "hosts"},
        {"source": "vps",         "target": "fastapi",     "rel": "hosts"},
        {"source": "vps",         "target": "nginx",       "rel": "hosts"},
        {"source": "vps",         "target": "ollama",      "rel": "hosts"},
        {"source": "docker",      "target": "fastapi",     "rel": "orchestrates"},
        {"source": "docker",      "target": "react",       "rel": "orchestrates"},
        {"source": "docker",      "target": "mysql",       "rel": "orchestrates"},
        {"source": "docker",      "target": "ollama",      "rel": "orchestrates"},
        {"source": "nginx",       "target": "react",       "rel": "proxies"},
        {"source": "nginx",       "target": "fastapi",     "rel": "proxies"},
        {"source": "fastapi",     "target": "mysql",       "rel": "uses"},
        {"source": "fastapi",     "target": "routes",      "rel": "includes"},
        {"source": "fastapi",     "target": "services",    "rel": "uses"},
        {"source": "fastapi",     "target": "auth",        "rel": "uses"},
        {"source": "routes",      "target": "copilot-api", "rel": "includes"},
        {"source": "services",    "target": "gemma-svc",   "rel": "includes"},
        {"source": "gemma-svc",   "target": "ollama",      "rel": "calls"},
        {"source": "ollama",      "target": "gemma",       "rel": "runs"},
        {"source": "ollama",      "target": "qwen",        "rel": "runs"},
        {"source": "react",       "target": "fastapi",     "rel": "calls"},
        {"source": "react",       "target": "pages",       "rel": "includes"},
        {"source": "react",       "target": "components",  "rel": "uses"},
        {"source": "claude-code", "target": "claude-md",   "rel": "reads"},
        {"source": "claude-code", "target": "memory",      "rel": "reads"},
        {"source": "claude-code", "target": "worktrees",   "rel": "uses"},
        {"source": "claude-code", "target": "fastapi",     "rel": "edits"},
        {"source": "claude-code", "target": "react",       "rel": "edits"},
        {"source": "claude-code", "target": "monitor",     "rel": "edits"},
        # Nuevos enlaces
        {"source": "hetzner",     "target": "fastapi",      "rel": "future-hosts"},
        {"source": "hetzner",     "target": "mysql",        "rel": "future-hosts"},
        {"source": "vps",         "target": "sigab-bot",   "rel": "hosts"},
        {"source": "fastapi",     "target": "sigab-bot",   "rel": "notifies"},
        {"source": "claude-code", "target": "skills-proj", "rel": "uses"},
        {"source": "claude-code", "target": "agents",      "rel": "orchestrates"},
        {"source": "claude-code", "target": "docs",        "rel": "reads"},
        {"source": "agents",      "target": "fastapi",     "rel": "edits"},
        {"source": "agents",      "target": "react",       "rel": "edits"},
        {"source": "docs",        "target": "memory",      "rel": "supplements"},
    ]
