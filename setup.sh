...
sudo mysql -e "ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY 'REDACTED_DB_ROOT_PASS';" 2>/dev/null || true

mysql -u root -pREDACTED_DB_ROOT_PASS -e "
...
CREATE USER IF NOT EXISTS 'sigah_user'@'localhost' IDENTIFIED BY 'REDACTED_DB_PASS';
CREATE USER IF NOT EXISTS 'sigah_user'@'127.0.0.1' IDENTIFIED BY 'REDACTED_DB_PASS';
...
if [ -f database/sigah_schema_fresh.sql ]; then
    mysql -u sigah_user -pREDACTED_DB_PASS sigah < database/sigah_schema_fresh.sql 2>/dev/null
...
for migration in database/migrations/*.sql; do
    if [ -f "$migration" ]; then
        mysql -u sigah_user -pREDACTED_DB_PASS sigah < "$migration" 2>/dev/null || true
...
if [ -f database/seed_data.sql ]; then
    mysql -u sigah_user -pREDACTED_DB_PASS sigah < database/seed_data.sql 2>/dev/null || true
fi
...
if [ -n "$BD_REAL" ]; then
    ...
    mysql -u sigah_user -pREDACTED_DB_PASS dummyequipomedicoimss < "$BD_REAL" 2>/dev/null || true
...
SIGAH_DB_PASS=REDACTED_DB_PASS
SIGAH_DB_NAME=sigah
SIGAH_SSL_DISABLED=true
SIGAH_JWT_SECRET=REDACTED_JWT_SECRET
