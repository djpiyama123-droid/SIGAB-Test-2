...
INSERT IGNORE INTO sigah.equipos (serie,marca,modelo,nombre,numero_inventario,piso,area,unidad,estado,criticidad,fecha_instalacion,foto_path,observaciones)
...
UPDATE sigah.equipos eq JOIN (SELECT m.Serie,MAX(m.FechaServicio) AS ultimo FROM dummyequipomedicoimss.mantenimientos m WHERE m.FechaServicio IS NOT NULL GROUP BY m.Serie) h ON h.Serie=eq.serie SET eq.fecha_ultimo_mantenimiento=h.ultimo WHERE eq.fecha_ultimo_mantenimiento IS NULL;

INSERT IGNORE INTO sigah.ubicaciones (nombre,piso,area,unidad)
...
SELECT COUNT(*) AS total_equipos FROM sigah.equipos;
SELECT piso, area, COUNT(*) AS equipos FROM sigah.equipos GROUP BY piso, area ORDER BY piso, area LIMIT 25;
