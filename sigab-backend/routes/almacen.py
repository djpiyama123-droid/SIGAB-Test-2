...
async def ajustar_stock(
    ...
    async with conn.cursor() as cur:
        # Verificar ownership por tenant antes de mutar (404, nunca 403)
        await cur.execute(
            "SELECT cantidad_disponible FROM refacciones_almacen WHERE id = %s AND tenant_id = %s",
            (id, tenant_id)
        )
        ...
        await cur.execute(
            "UPDATE refacciones_almacen SET cantidad_disponible = %s WHERE id = %s AND tenant_id = %s",
            (nueva, id, tenant_id)
        )
