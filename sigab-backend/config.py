"""
config.py ??? Variables de configuraci??n centralizadas del backend SIGAH.
...
Secciones:
  - DB_CONFIG:         Conexi??n MySQL (host, port, user, pass, db)
  - JWT:               Secreto, algoritmo, TTL de acceso y refresh
  - PUBLIC_BASE_URL:   URL embebida en QRs (IP LAN en producci??n)
  - CORS_EXTRA:        Or??genes adicionales para celulares en LAN
  - OLLAMA/GEMMA:      IA local v??a Ollama (Gemma 4B / Qwen 7B)
...
# ?????? Autenticaci??n JWT ???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
# SIGAH_JWT_SECRET es OBLIGATORIO en producci??n. En dev cae a un valor
...
# URL base p??blica para embeber en QR. Puede ser IP LAN, hostname o dominio.
