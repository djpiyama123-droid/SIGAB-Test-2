...
| Accesibilidad transversal — `<label>` asociado a su control de formulario (Reservas, Órdenes, Equipos, Almacén, Metrología, Copilot, Trazabilidad, Capacitaciones, Preventivos, Órdenes formato IMSS, LandingPage, TripleValidationModal, ChangePasswordModal) | 🟡 Parcial | v4.0.59 / v4.0.60 / v4.0.61 / v4.0.62 / v4.0.63 / v4.0.64 / v4.0.65 / v4.0.66 / v4.0.67 / v4.0.68 / v4.0.69 / v4.0.70 / v4.0.71 / v4.0.72 / v4.0.73 / (este ciclo, v4.0.74) | Séptimo barrido de accesibilidad (distinto de los 6 anteriores: `aria-label` de botones de cerrar, `title` de iconos, navegación por teclado en filas/tarjetas, Escape, focus trap, botones sin nombre de Copilot). Hallazgo: `grep -rn "<label" sigab-frontend/src` sin `htmlFor` devolvió 117 líneas en decenas de archivos — cada `<label>Texto</label>` es visualmente correcto pero no está asociado programáticamente a su `<input>/<select>/<textarea>` hermano, así que un lector de pantalla anuncia el campo sin nombre al enfocarlo con Tab. Alcance demasiado grande para un ciclo (regla del proyecto: un módulo a la vez) → v4.0.59 cerró `pages/Reservas.jsx` (`NuevaReservaModal`), v4.0.60 cerró `pages/Ordenes.jsx` (formulario "Nueva Orden de Servicio"), v4.0.61 cerró `components/EquipoForm.jsx`, v4.0.62 cerró `components/OrdenServicioRapidaModal.jsx`, v4.0.63 cerró `components/NuevaOrdenModal.jsx`, v4.0.64 cerró `components/EventoAdversoModal.jsx`, v4.0.65 cerró `pages/Almacen.jsx`, v4.0.66 cerró `pages/Metrologia.jsx`, v4.0.67 cerró `pages/Copilot.jsx`, v4.0.68 cerró `pages/Trazabilidad.jsx`, v4.0.69 cerró `pages/Capacitaciones.jsx`, v4.0.70 cerró `components/PreventivoForm.jsx`, v4.0.71 cerró `components/OrdenCasillasForm.jsx`, v4.0.72 cerró `pages/LandingPage.jsx`, v4.0.73 cerró `components/TripleValidationModal.jsx` (ver detalle de esas versiones). Ciclo 2026-07-22 (loop-thinkcentre, v4.0.74): `components/ChangePasswordModal.jsx` — era el candidato explícito que quedó del cierre de v4.0.73 (empatado en 3 con TripleValidationModal, ya resuelto). Los 3 `<label>` del modal (Contraseña Actual, Nueva Contraseña, Confirmar Nueva Contraseña) eran simples, sin grupos de radio/checkbox → se agregó `id`/`htmlFor` a los 3 (`change-password-actual`, `change-password-nueva`, `change-password-confirmar`). Cambio 100% aditivo, sin tocar clases/layout/la lógica de validación (incluye el modo `required` obligatorio post-login). Verificado con `npm run build` + `npx vitest run` (62 tests) en verde. **Pendiente**: `pages/SuperAdmin.jsx`/`pages/ChecklistPage.jsx`/`components/OCRScannerModal.jsx` (2 c/u) son los candidatos explícitos del próximo barrido; después `pages/Equipos.jsx`/`components/EquipoForm.jsx`/`components/OrdenServicioRapidaModal.jsx`/`components/Copilot.jsx` (1 c/u, ya wrapping-label válido a confirmar caso por caso) — un módulo por ciclo. |
