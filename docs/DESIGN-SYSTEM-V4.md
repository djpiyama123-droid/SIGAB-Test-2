...
| Accesibilidad transversal — `<label>` asociado a su control de formulario (Reservas, Órdenes, Equipos, Almacén, Metrología, Copilot, Trazabilidad, Capacitaciones, Preventivos, Órdenes formato IMSS) | 🟡 Parcial | v4.0.59 / v4.0.60 / v4.0.61 / v4.0.62 / v4.0.63 / v4.0.64 / v4.0.65 / v4.0.66 / v4.0.67 / v4.0.68 / v4.0.69 / v4.0.70 / (este ciclo, v4.0.71) | Séptimo barrido de accesibilidad (distinto de los 6 anteriores: `aria-label` de botones de cerrar, `title` de iconos, navegación por teclado en filas/tarjetas, Escape, focus trap, botones sin nombre de Copilot). Hallazgo: `grep -rn "<label" sigab-frontend/src` sin `htmlFor` devolvió 117 líneas en decenas de archivos — cada `<label>Texto</label>` es visualmente correcto pero no está asociado programáticamente a su `<input>/<select>/<textarea>` hermano, así que un lector de pantalla anuncia el campo sin nombre al enfocarlo con Tab. Alcance demasiado grande para un ciclo (regla del proyecto: un módulo a la vez) → v4.0.59 cerró `pages/Reservas.jsx` (`NuevaReservaModal`), v4.0.60 cerró `pages/Ordenes.jsx` (formulario "Nueva Orden de Servicio"), v4.0.61 cerró `components/EquipoForm.jsx`, v4.0.62 cerró `components/OrdenServicioRapidaModal.jsx`, v4.0.63 cerró `components/NuevaOrdenModal.jsx`, v4.0.64 cerró `components/EventoAdversoModal.jsx`, v4.0.65 cerró `pages/Almacen.jsx`, v4.0.66 cerró `pages/Metrologia.jsx`, v4.0.67 cerró `pages/Copilot.jsx`, v4.0.68 cerró `pages/Trazabilidad.jsx`, v4.0.69 cerró `pages/Capacitaciones.jsx`, v4.0.70 cerró `components/PreventivoForm.jsx` (ver detalle de esas versiones). Ciclo 2026-07-21 (loop-thinkcentre, v4.0.71): `components/OrdenCasillasForm.jsx` — el candidato explícito que dejó apuntado el ciclo anterior. Al inspeccionar los 2 `<label>` de `:147,178` (dentro de `RadioGroup`/`CheckboxGrid`) se confirmó que YA están correctamente asociados: envuelven su `<input type="radio"/checkbox">` como hijo directo (el patrón "wrapping label" es una asociación programática válida sin necesidad de `htmlFor`/`id`, no requerían cambio) — el ciclo anterior los había dejado como pendientes de inspección "por si eran grupos de checkbox/radio" y en efecto lo eran, y por eso ya estaban bien. Los únicos 2 bugs reales eran los `<label>` simples de `:478,490` ("Observaciones breves"/"Refacciones solicitadas", ambos `<textarea>` del Bloque F "Texto libre") → se agregó `id`/`htmlFor` (`oc-observaciones-breves`, `oc-refacciones-solicitadas`). Con esto, un `grep -rn "<label" sigab-frontend/src \| grep -v htmlFor` ya no devuelve ninguna línea de este archivo. De paso, `npx vitest run` reveló un bug real preexistente no relacionado con el barrido: `utils/preventivos.js:diasRestantes()` podía devolver `-0` en vez de `0` para una fecha de "hoy" (el `Math.ceil` de una fracción de día ligeramente negativa, por el desfase de milisegundos entre el `new Date(fecha)` recibido y el `new Date()` interno evaluado un instante después) — no rompe ninguna comparación real en la app (`-0 === 0` es `true` en JS) pero sí el test estricto (`Object.is`) y es una inconsistencia de valor; se normalizó con `diff === 0 ? 0 : diff`. Verificado con `npm run build` + `npx vitest run` (62 tests) en verde. **Pendiente**: `pages/LandingPage.jsx`/`components/TripleValidationModal.jsx`/`components/ChangePasswordModal.jsx` (3 c/u) son los mayores candidatos del próximo barrido; después `pages/SuperAdmin.jsx`/`pages/ChecklistPage.jsx`/`components/OCRScannerModal.jsx` (2 c/u), `pages/Equipos.jsx`/`components/EquipoForm.jsx`/`components/OrdenServicioRapidaModal.jsx`/`components/Copilot.jsx` (1 c/u, ya wrapping-label válido a confirmar caso por caso) — un módulo por ciclo. |
