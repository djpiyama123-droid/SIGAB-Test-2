...
| Accesibilidad transversal — `<label>` asociado a su control de formulario (Reservas, Órdenes, Equipos, Almacén, Metrología, Copilot, Trazabilidad, Capacitaciones, Preventivos, Órdenes formato IMSS, LandingPage, TripleValidationModal, ChangePasswordModal, SuperAdmin) | 🟡 Parcial | v4.0.59 / v4.0.60 / v4.0.61 / v4.0.62 / v4.0.63 / v4.0.64 / v4.0.65 / v4.0.66 / v4.0.67 / v4.0.68 / v4.0.69 / v4.0.70 / v4.0.71 / v4.0.72 / v4.0.73 / v4.0.74 / (este ciclo, v4.0.75) | Séptimo barrido de accesibilidad (distinto de los 6 anteriores: `aria-label` de botones de cerrar, `title` de iconos, navegación por teclado en filas/tarjetas, Escape, focus trap, botones sin nombre de Copilot). Hallazgo: `grep -rn "<label" sigab-frontend/src` sin `htmlFor` devolvió 117 líneas en decenas de archivos — cada `<label>Texto</label>` es visualmente correcto pero no está asociado programáticamente a su `<input>/<select>/<textarea>` hermano, así que un lector de pantalla anuncia el campo sin nombre al enfocarlo con Tab. Alcance demasiado grande para un ciclo (regla del proyecto: un módulo a la vez) → v4.0.59 cerró `pages/Reservas.jsx` (`NuevaReservaModal`), v4.0.60 cerró `pages/Ordenes.jsx` (formulario "Nueva Orden de Servicio"), v4.0.61 cerró `components/EquipoForm.jsx`, v4.0.62 cerró `components/OrdenServicioRapidaModal.jsx`, v4.0.63 cerró `components/NuevaOrdenModal.jsx`, v4.0.64 cerró `components/EventoAdversoModal.jsx`, v4.0.65 cerró `pages/Almacen.jsx`, v4.0.66 cerró `pages/Metrologia.jsx`, v4.0.67 cerró `pages/Copilot.jsx`, v4.0.68 cerró `pages/Trazabilidad.jsx`, v4.0.69 cerró `pages/Capacitaciones.jsx`, v4.0.70 cerró `components/PreventivoForm.jsx`, v4.0.71 cerró `components/OrdenCasillasForm.jsx`, v4.0.72 cerró `pages/LandingPage.jsx`, v4.0.73 cerró `components/TripleValidationModal.jsx`, v4.0.74 cerró `components/ChangePasswordModal.jsx` (ver detalle de esas versiones). Ciclo 2026-07-22 (loop-thinkcentre, v4.0.75): `pages/SuperAdmin.jsx` — uno de los 3 candidatos explícitos que quedaron del cierre de v4.0.74 (junto con `ChecklistPage.jsx` y `OCRScannerModal.jsx`, 2 c/u); se eligió por ser el caso más simple de los tres (2 `<input type="text">` planos del formulario "Nuevo Registro" de tenants, sin grupos de radio ni contenido dinámico). Se agregó `id`/`htmlFor` a los 2 (`admin-tenant-nombre`, `admin-tenant-slug`). Cambio 100% aditivo, sin tocar clases/layout/lógica. Verificado con `npm run build` + `npx vitest run` (62 tests) en verde. **Pendiente**: `pages/ChecklistPage.jsx` (radio group SI/NO/N-A por pregunta dinámica + 1 label simple de "Observaciones Adicionales") y `components/OCRScannerModal.jsx` (labels dinámicos por campo extraído por OCR, más un tercer `<label>` de "Refacciones detectadas" que en realidad no está asociado a ningún control — es un encabezado de lista, candidato a revisar si debe seguir siendo `<label>` o pasar a `<p>`/`<h4>`) son los candidatos explícitos del próximo barrido; después `pages/Equipos.jsx`/`components/EquipoForm.jsx`/`components/OrdenServicioRapidaModal.jsx`/`components/Copilot.jsx` (1 c/u, ya wrapping-label válido a confirmar caso por caso) — un módulo por ciclo. |
