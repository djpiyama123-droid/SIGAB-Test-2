...
| Accesibilidad transversal — `<label>` asociado a su control de formulario (Reservas) | 🟡 Parcial | (este ciclo, v4.0.59) | Séptimo barrido de accesibilidad (distinto de los 6 anteriores: `aria-label` de botones de cerrar, `title` de iconos, navegación por teclado en filas/tarjetas, Escape, focus trap, botones sin nombre de Copilot). Hallazgo: `grep -rn "<label" sigab-frontend/src` sin `htmlFor` devolvió 117 líneas en decenas de archivos — cada `<label>Texto</label>` es visualmente correcto pero no está asociado programáticamente a su `<input>/<select>/<textarea>` hermano, así que un lector de pantalla anuncia el campo sin nombre al enfocarlo con Tab. Alcance demasiado grande para un ciclo (regla del proyecto: un módulo a la vez) → se cerró **solo** `pages/Reservas.jsx` (`NuevaReservaModal`, el único punto de entrada para crear/editar una reserva), candidato explícito del punto 3 de este backlog. Los 6 `<label>` del formulario ahora llevan `htmlFor`/`id` (`reserva-equipo`, `reserva-area`, `reserva-piso`, `reserva-fecha-inicio`, `reserva-fecha-fin`, `reserva-motivo`); el input de filtro de equipo (que solo tenía `placeholder`, no un nombre accesible válido) recibió `aria-label="Buscar equipo por nombre, serie o modelo"` porque el `htmlFor` del label visible se ligó al `<select>` real (el control que determina el valor enviado), no al filtro. Cambio 100% aditivo, sin tocar clases/layout/lógica. **Pendiente**: ~111 líneas de `<label>` sin `htmlFor` en el resto de la app (`Copilot.jsx`, `Ordenes.jsx`, `Metrologia.jsx`, `ChecklistPage.jsx`, `Equipos.jsx` y otros) — candidato del próximo barrido, un módulo por ciclo. |
