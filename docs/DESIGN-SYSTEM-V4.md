...
| Detalle de Equipo | 🟡 Parcial | v4.0.16 / (este ciclo) | Se corrigió bug de contraste en `EquipoDetail.jsx`, `EquipoForm.jsx` y `HistorialEquipoModal.jsx`: campos "Criticidad", "N° Contrato/Servicio", "Proveedor", traslados, tabs e historial (órdenes/preventivos) usaban `text-white` fijo como texto de cuerpo → ahora `var(--content-text)`. Badges de estado siguen con `text-white` fijo (correcto, van sobre fondo de color). v4.0.16: nueva sección "Próximo Mantenimiento" con vista de calendario (`CalendarioMantenimiento.jsx`), adaptando el patrón visual del heatmap de Reservas a un mes con el día objetivo resaltado por urgencia — petición directa de Gustavo (punto 5/5 de Inventario). Ciclo 2026-07-12 (loop-thinkcentre): los 6 botones del footer de acciones (Eliminar, Nueva OS, Historial, QR, Cerrar, Editar) medían ~36px de alto (`py-2` + `text-sm`), por debajo del mínimo táctil 44px de este documento — mismo patrón ya corregido en Dashboard (v4.0.35) y Reservas (v4.0.38) → se agregó `min-h-[44px] justify-center` a los 6 (el botón "Cerrar" no tenía `flex`, se agregó `flex items-center` de paso para que el min-height centre el texto). El `<select>` inline de cambio rápido de estado (badge pill, `text-xs`/`py-1`) se dejó igual a propósito: sigue el patrón de badges/pills compactos ya establecido en toda la app (no es un CTA primario). Ciclo 2026-07-23 (loop-thinkcentre, v4.0.77): el screen ID móvil de Stitch (`89c7d513601244398362c3c40a9ba86e`) pide explícitamente "CTA fija" — el footer de acciones vivía en el mismo contenedor `overflow-auto` que el resto del modal, así que en equipos con contenido largo (fotos, documentos, varias órdenes/traslados) había que hacer scroll hasta el final para llegar a los botones → el contenedor del modal pasó a `flex flex-col overflow-hidden` con header/footer `flex-shrink-0` y solo el cuerpo intermedio con `flex-1 overflow-y-auto`; el footer de 6 acciones ahora es una barra fija siempre visible. Cambio 100% de layout/CSS, sin tocar lógica. Ciclo 2026-07-23 (loop-thinkcentre, v4.0.78): mismo screen ID pide también "tabs expediente, timeline" — el cuerpo del modal era una sola columna con 7 secciones apiladas (info+QR, contrato, calendario, galería, documentos, tickets/OS, traslados), obligando a scrollear todo el expediente para llegar al historial de órdenes. Se dividió en 2 pestañas (`role="tablist"`/`role="tab"`/`role="tabpanel"`, patrón WAI-ARIA básico sin roving tabindex — no hay precedente de tabs en el resto de la app): "Expediente" (info, QR, contrato, calendario de mantenimiento, galería, documentos) e "Historial" (tickets/OS + traslados, con contador `(N)` en la pestaña). El fetch de datos (`recargarHistorial`/`getExpedienteEquipo`) no cambió — sigue disparándose una vez al abrir el modal, las pestañas solo alternan qué panel se renderiza (`hidden`), así que cambiar de pestaña no dispara peticiones nuevas ni pierde el estado ya cargado. Footer de acciones y header siguen fijos (v4.0.77), la barra de tabs también es `flex-shrink-0`. Ciclo 2026-07-23 (loop-thinkcentre, v4.0.79): al revisar v4.0.78 se encontró que el footer todavía tenía el botón "🕒 Historial" que abría `HistorialEquipoModal.jsx` completo ENCIMA del detalle ya abierto (modal sobre modal), duplicando Órdenes/Traslados que la pestaña "Historial" ya mostraba y solo aportando su pestaña "Preventivos" — dos UIs de historial apiladas, contrario a la intención de la pestaña. Se movió Preventivos a la pestaña "Historial" inline (nueva sección "Preventivos Programados", mismo patrón de tarjeta, contador `(N)` ahora suma las 3 categorías) y se eliminó el botón/estado/import de `HistorialEquipoModal` en `EquipoDetail.jsx` (footer: 5 botones en vez de 6). `HistorialEquipoModal.jsx` no se tocó — sigue en uso, sin anidar, desde `HospitalMap.jsx`. Pendiente: el resto del layout Stitch completo (variantes desktop 2562px/2386px, screen ID `f9b01c3c7232494e9bb95899c10d40b2`) y las 4 variantes de Reservas sin elegir |
...
| Accesibilidad transversal — `<label>` asociado a su control de formulario (Reservas, Órdenes, Equipos, Almacén, Metrología, Copilot, Trazabilidad, Capacitaciones, Preventivos, Órdenes formato IMSS, LandingPage, TripleValidationModal, ChangePasswordModal, SuperAdmin, ChecklistPage, OCRScannerModal) | ✅ Aplicado | v4.0.59 … v4.0.75 / (este ciclo, v4.0.76) | Séptimo barrido de accesibilidad (distinto de los 6 anteriores: `aria-label` de botones de cerrar, `title` de iconos, navegación por teclado en filas/tarjetas, Escape, focus trap, botones sin nombre de Copilot). Hallazgo: `grep -rn "<label" sigab-frontend/src` sin `htmlFor` devolvió 117 líneas en decenas de archivos — cada `<label>Texto</label>` es visualmente correcto pero no está asociado programáticamente a su `<input>/<select>/<textarea>` hermano, así que un lector de pantalla anuncia el campo sin nombre al enfocarlo con Tab. Alcance demasiado grande para un ciclo (regla del proyecto: un módulo a la vez) → v4.0.59 cerró `pages/Reservas.jsx` (`NuevaReservaModal`), v4.0.60 cerró `pages/Ordenes.jsx` (formulario "Nueva Orden de Servicio"), v4.0.61 cerró `components/EquipoForm.jsx`, v4.0.62 cerró `components/OrdenServicioRapidaModal.jsx`, v4.0.63 cerró `components/NuevaOrdenModal.jsx`, v4.0.64 cerró `components/EventoAdversoModal.jsx`, v4.0.65 cerró `pages/Almacen.jsx`, v4.0.66 cerró `pages/Metrologia.jsx`, v4.0.67 cerró `pages/Copilot.jsx`, v4.0.68 cerró `pages/Trazabilidad.jsx`, v4.0.69 cerró `pages/Capacitaciones.jsx`, v4.0.70 cerró `components/PreventivoForm.jsx`, v4.0.71 cerró `components/OrdenCasillasForm.jsx`, v4.0.72 cerró `pages/LandingPage.jsx`, v4.0.73 cerró `components/TripleValidationModal.jsx`, v4.0.74 cerró `components/ChangePasswordModal.jsx`, v4.0.75 cerró `pages/SuperAdmin.jsx` (ver detalle de esas versiones). **Ciclo de cierre 2026-07-22 (loop-thinkcentre, v4.0.76)**: los 2 candidatos explícitos que quedaban del cierre de v4.0.75 — `pages/ChecklistPage.jsx` y `components/OCRScannerModal.jsx`. Al leer `ChecklistPage.jsx` de cerca, el radio group SI/NO/N-A por pregunta (`<label><input type="radio".../><span>{val}</span></label>`) resultó ya ser un wrapping-label válido (el `<input>` vive dentro del `<label>`, asociación implícita correcta, sin bug) — el único hallazgo real ahí era el `<label>` de "Observaciones Adicionales" sin `htmlFor`, corregido con `id="checklist-observaciones"`. En `OCRScannerModal.jsx` se agregó `id`/`htmlFor` dinámico (`ocr-field-${key}`, la `key` es un nombre de campo fijo en snake_case tomado de `FIELD_LABELS`, seguro como id) a los labels de los campos extraídos por OCR editables; el tercer `<label>` "Refacciones detectadas (N)" no envolvía ni apuntaba a ningún control de formulario — es un encabezado de lista (`<ul>` de refacciones), mal etiquetado como `<label>` desde su creación → cambiado a `<p>` (mismas clases, cero cambio visual). De paso, al re-auditar los wrapping-labels restantes del grep (`Equipos.jsx`, `Copilot.jsx`, `OrdenCasillasForm.jsx` ×2, `EquipoForm.jsx`, `OrdenServicioRapidaModal.jsx` ×3) se confirmó que los 8 ya envuelven su propio `<input>`/checkbox/radio — ningún bug real pendiente. **Con esto se cierra por completo la categoría**: `grep -rn "<label" sigab-frontend/src` ya no devuelve ningún `<label>` sin asociación (ni por `htmlFor` ni por wrapping) a un control real. Verificado con `npm run build` + `npx vitest run` (62 tests) en verde. |
...
3. Aplicar el mismo criterio de "sin colores fijos que dependan del fondo"
   ...
   Avance 2026-07-23 (v4.0.77): CTA fija del footer de Detalle de Equipo.
   Avance 2026-07-23 (v4.0.78): tabs Expediente/Historial de Detalle de
   Equipo (ver tabla de avance). Avance 2026-07-23 (v4.0.79): la pestaña
   Historial ganó Preventivos y se quitó el botón redundante que abría
   `HistorialEquipoModal` encima del detalle (modal sobre modal) — sigue
   pendiente el resto del layout (variantes desktop) y las 4 variantes de
   Reservas sin elegir.
