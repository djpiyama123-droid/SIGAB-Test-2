...
| Accesibilidad transversal — `<label>` asociado a su control de formulario (Reservas, Órdenes, Equipos) | 🟡 Parcial | v4.0.59 / v4.0.60 / (este ciclo, v4.0.61) | Séptimo barrido de accesibilidad (distinto de los 6 anteriores: `aria-label` de botones de cerrar, `title` de iconos, navegación por teclado en filas/tarjetas, Escape, focus trap, botones sin nombre de Copilot). Hallazgo: `grep -rn "<label" sigab-frontend/src` sin `htmlFor` devolvió 117 líneas en decenas de archivos — cada `<label>Texto</label>` es visualmente correcto pero no está asociado programáticamente a su `<input>/<select>/<textarea>` hermano, así que un lector de pantalla anuncia el campo sin nombre al enfocarlo con Tab. Alcance demasiado grande para un ciclo (regla del proyecto: un módulo a la vez) → v4.0.59 cerró `pages/Reservas.jsx` (`NuevaReservaModal`), v4.0.60 cerró `pages/Ordenes.jsx` (formulario "Nueva Orden de Servicio", ver detalle de esa versión). Ciclo 2026-07-19 (loop-thinkcentre, v4.0.61): `components/EquipoForm.jsx` (modal de crear/editar equipo, candidato explícito de la lista pendiente y parte directa de la meta de cierre — pantalla Equipos/Inventario). A diferencia de Reservas/Ordenes (11-15 `<label>` sueltos editados uno por uno), este archivo canaliza sus 18 campos por un único componente interno `Campo({ label, error, children })` donde el `<label>` es hermano de `children` (el `<input>`/`<select>` real), no su envoltorio — así que se corrigió una sola vez en la fuente: `Campo` ahora genera un `id` determinístico a partir del texto de `label` (helper `slugify`, sin acentos/símbolos, ej. "N° Serie *" → `equipo-form-n-serie`), lo pasa como `htmlFor` al `<label>` y lo inyecta en el control hijo vía `cloneElement(children, { id })` — un cambio de ~10 líneas resuelve los 18 campos (Nombre, Inventario/ID interno, N° Serie, Marca, Modelo, Tipo de equipo, Estado, Criticidad, Clase COFEPRIS, Área, Piso, Ubicación física, Zona del mapa, Tipo de adquisición, Fecha de compra, Próximo mantenimiento, Proveedor de servicio, N° Contrato de servicio) sin tocar ninguno de los 18 sitios de uso de `<Campo>`. Se revisaron los otros 2 `<label>` del archivo (fuera de `Campo`): el de la galería de fotos ya envuelve su `<input type="file">` (asociación implícita, sin bug) — no se tocó. `pages/Equipos.jsx` también se revisó (candidato de la lista pendiente): su único `<label>` envuelve el checkbox "Fotos incompletas" (asociación implícita, sin bug) — no queda pendiente en ese archivo. Cambio 100% aditivo (atributos `id`/`htmlFor` generados, sin tocar clases/layout/lógica de negocio ni el payload que se envía al backend). **Pendiente**: ~90 líneas de `<label>` sin `htmlFor` en el resto de la app (`Copilot.jsx`, `Metrologia.jsx`, `ChecklistPage.jsx`, `Almacen.jsx`, `Trazabilidad.jsx`, `Capacitaciones.jsx`, `SuperAdmin.jsx`, `LandingPage.jsx` y varios componentes/modales compartidos como `EventoAdversoModal.jsx`, `NuevaOrdenModal.jsx`, `OrdenServicioRapidaModal.jsx`, `PreventivoForm.jsx`, `TripleValidationModal.jsx`, `ChangePasswordModal.jsx`, `OCRScannerModal.jsx`, `OrdenCasillasForm.jsx`) — candidato del próximo barrido, un módulo por ciclo. |
