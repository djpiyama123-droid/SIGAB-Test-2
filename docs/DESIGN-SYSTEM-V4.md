...
| Accesibilidad transversal — `<label>` asociado a su control de formulario (Reservas, Órdenes, Equipos, Almacén, Metrología, Copilot, Trazabilidad, Capacitaciones, Preventivos, Órdenes formato IMSS, LandingPage) | 🟡 Parcial | v4.0.59 / v4.0.60 / v4.0.61 / v4.0.62 / v4.0.63 / v4.0.64 / v4.0.65 / v4.0.66 / v4.0.67 / v4.0.68 / v4.0.69 / v4.0.70 / v4.0.71 / (este ciclo, v4.0.72) | Séptimo barrido de accesibilidad (distinto de los 6 anteriores: `aria-label` de botones de cerrar, `title` de iconos, navegación por teclado en filas/tarjetas, Escape, focus trap, botones sin nombre de Copilot). Hallazgo: `grep -rn "<label" sigab-frontend/src` sin `htmlFor` devolvió 117 líneas en decenas de archivos — cada `<label>Texto</label>` es visualmente correcto pero no está asociado programáticamente a su `<input>/<select>/<textarea>` hermano, así que un lector de pantalla anuncia el campo sin nombre al enfocarlo con Tab. Alcance demasiado grande para un ciclo (regla del proyecto: un módulo a la vez) → v4.0.59 cerró `pages/Reservas.jsx` (`NuevaReservaModal`), v4.0.60 cerró `pages/Ordenes.jsx` (formulario "Nueva Orden de Servicio"), v4.0.61 cerró `components/EquipoForm.jsx`, v4.0.62 cerró `components/OrdenServicioRapidaModal.jsx`, v4.0.63 cerró `components/NuevaOrdenModal.jsx`, v4.0.64 cerró `components/EventoAdversoModal.jsx`, v4.0.65 cerró `pages/Almacen.jsx`, v4.0.66 cerró `pages/Metrologia.jsx`, v4.0.67 cerró `pages/Copilot.jsx`, v4.0.68 cerró `pages/Trazabilidad.jsx`, v4.0.69 cerró `pages/Capacitaciones.jsx`, v4.0.70 cerró `components/PreventivoForm.jsx`, v4.0.71 cerró `components/OrdenCasillasForm.jsx` (ver detalle de esas versiones). Ciclo 2026-07-21 (loop-thinkcentre, v4.0.72): `pages/LandingPage.jsx` — uno de los 3 candidatos que dejó apuntados el ciclo anterior (empataba en volumen con `TripleValidationModal.jsx`/`ChangePasswordModal.jsx`). Los 3 `<label>` del formulario público "Agenda tu auditoría gratuita NOM-016" (Nombre completo, Institución médica, Correo corporativo) eran simples, sin grupos de radio/checkbox → se agregó `id`/`htmlFor` a los 3 (`landing-nombre`, `landing-institucion`, `landing-correo`). Cambio 100% aditivo, sin tocar clases/layout/el `onSubmit` (`goLogin`). Verificado con `npm run build` + `npx vitest run` (62 tests) en verde. **Pendiente**: `components/TripleValidationModal.jsx`/`components/ChangePasswordModal.jsx` (3 c/u) son los mayores candidatos del próximo barrido; después `pages/SuperAdmin.jsx`/`pages/ChecklistPage.jsx`/`components/OCRScannerModal.jsx` (2 c/u), `pages/Equipos.jsx`/`components/EquipoForm.jsx`/`components/OrdenServicioRapidaModal.jsx`/`components/Copilot.jsx` (1 c/u, ya wrapping-label válido a confirmar caso por caso) — un módulo por ciclo. |
