...
| Accesibilidad transversal — `<label>` asociado a su control de formulario (Reservas, Órdenes, Equipos) | 🟡 Parcial | v4.0.59 / v4.0.60 / v4.0.61 / v4.0.62 / (este ciclo, v4.0.63) | Séptimo barrido de accesibilidad (distinto de los 6 anteriores: `aria-label` de botones de cerrar, `title` de iconos, navegación por teclado en filas/tarjetas, Escape, focus trap, botones sin nombre de Copilot). Hallazgo: `grep -rn "<label" sigab-frontend/src` sin `htmlFor` devolvió 117 líneas en decenas de archivos — cada `<label>Texto</label>` es visualmente correcto pero no está asociado programáticamente a su `<input>/<select>/<textarea>` hermano, así que un lector de pantalla anuncia el campo sin nombre al enfocarlo con Tab. Alcance demasiado grande para un ciclo (regla del proyecto: un módulo a la vez) → v4.0.59 cerró `pages/Reservas.jsx` (`NuevaReservaModal`), v4.0.60 cerró `pages/Ordenes.jsx` (formulario "Nueva Orden de Servicio"), v4.0.61 cerró `components/EquipoForm.jsx`, v4.0.62 cerró `components/OrdenServicioRapidaModal.jsx` (ver detalle de esas versiones). Ciclo 2026-07-19 (loop-thinkcentre, v4.0.63): `components/NuevaOrdenModal.jsx` (modal reutilizable de "Nueva Orden de Servicio" usado desde `Ordenes.jsx`, `FichaTecnica` del mapa y `EquipoDetail.jsx` — mismo módulo Órdenes, el mayor candidato por volumen tras cerrar `OrdenServicioRapidaModal.jsx`). Se agregó `id`/`htmlFor` a los 11 campos con label+input/select/textarea hermano (equipo nombre/serie, técnico, área, piso, tipo de mantenimiento, prioridad, localización completa, falla reportada, hora de inicio/término, tiempo estimado/real, recibe de conformidad nombre/matrícula), incluyendo los 5 campos que se generan dinámicamente vía `.map()` (id derivado de la clave del campo, `nueva-orden-${k}`) y los 2 `<select>` (Tipo, Prioridad — este archivo, a diferencia de `OrdenServicioRapidaModal.jsx`, usa un `<select>` simple para prioridad, no un grupo de radios, así que no aplicó el patrón `fieldset`/`legend`). Cambio 100% aditivo (solo atributos `id`/`htmlFor`), sin tocar clases/layout/lógica de negocio ni el payload enviado al backend. Verificado con `npm run build` + `npx vitest run` (62 tests) en verde. **Pendiente**: `components/EventoAdversoModal.jsx` (13) es ahora el mayor por volumen; después `pages/Almacen.jsx` (9), `pages/Metrologia.jsx`/`pages/Copilot.jsx` (6 c/u), `pages/Trazabilidad.jsx`/`pages/Capacitaciones.jsx` (5 c/u), `components/PreventivoForm.jsx`/`components/OrdenCasillasForm.jsx` (4 c/u), `pages/LandingPage.jsx`/`components/TripleValidationModal.jsx`/`components/ChangePasswordModal.jsx` (3 c/u), `pages/SuperAdmin.jsx`/`pages/ChecklistPage.jsx`/`components/OCRScannerModal.jsx` (2 c/u) — candidato del próximo barrido, un módulo por ciclo. |
