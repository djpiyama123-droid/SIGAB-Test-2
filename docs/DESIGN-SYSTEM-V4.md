...
| Accesibilidad transversal — `<label>` asociado a su control de formulario (Reservas, Órdenes, Equipos, Almacén, Metrología, Copilot, Trazabilidad, Capacitaciones, Preventivos) | 🟡 Parcial | v4.0.59 / v4.0.60 / v4.0.61 / v4.0.62 / v4.0.63 / v4.0.64 / v4.0.65 / v4.0.66 / v4.0.67 / v4.0.68 / v4.0.69 / (este ciclo, v4.0.70) | Séptimo barrido de accesibilidad (distinto de los 6 anteriores: `aria-label` de botones de cerrar, `title` de iconos, navegación por teclado en filas/tarjetas, Escape, focus trap, botones sin nombre de Copilot). Hallazgo: `grep -rn "<label" sigab-frontend/src` sin `htmlFor` devolvió 117 líneas en decenas de archivos — cada `<label>Texto</label>` es visualmente correcto pero no está asociado programáticamente a su `<input>/<select>/<textarea>` hermano, así que un lector de pantalla anuncia el campo sin nombre al enfocarlo con Tab. Alcance demasiado grande para un ciclo (regla del proyecto: un módulo a la vez) → v4.0.59 cerró `pages/Reservas.jsx` (`NuevaReservaModal`), v4.0.60 cerró `pages/Ordenes.jsx` (formulario "Nueva Orden de Servicio"), v4.0.61 cerró `components/EquipoForm.jsx`, v4.0.62 cerró `components/OrdenServicioRapidaModal.jsx`, v4.0.63 cerró `components/NuevaOrdenModal.jsx`, v4.0.64 cerró `components/EventoAdversoModal.jsx`, v4.0.65 cerró `pages/Almacen.jsx`, v4.0.66 cerró `pages/Metrologia.jsx`, v4.0.67 cerró `pages/Copilot.jsx`, v4.0.68 cerró `pages/Trazabilidad.jsx`, v4.0.69 cerró `pages/Capacitaciones.jsx` (ver detalle de esas versiones). Ciclo 2026-07-21 (loop-thinkcentre, v4.0.70): `components/PreventivoForm.jsx` (modal "Nuevo Preventivo") — uno de los 2 candidatos explícitos que dejó apuntado el ciclo anterior (empataba en volumen, 4, con `OrdenCasillasForm.jsx`; se eligió por no tener grupos de radio/botón que requieran `fieldset`/`legend`, a diferencia de los 2 `<label>` de `OrdenCasillasForm.jsx:147,178`). Se agregó `id`/`htmlFor` a los 4 campos: tipo de preventivo, próxima ejecución y frecuencia en días (los 3 `<input>`) y procedimiento (`<textarea>`). Cambio 100% aditivo (solo atributos `id`/`htmlFor`), sin tocar clases, layout, lógica de negocio ni el payload enviado a `api.crearPreventivo` — cero riesgo visual o funcional. Verificado con `npm run build` + `npx vitest run` (62 tests) en verde. **Pendiente**: `components/OrdenCasillasForm.jsx` (2 `<label>` simples en `Observaciones breves`/`Refacciones solicitadas` en `:478,490`, + 2 en `:147,178` que hay que inspeccionar con detalle por si son grupos de checkbox/radio) queda como el mayor candidato del próximo barrido; después `pages/LandingPage.jsx`/`components/TripleValidationModal.jsx`/`components/ChangePasswordModal.jsx` (3 c/u), `pages/SuperAdmin.jsx`/`pages/ChecklistPage.jsx`/`components/OCRScannerModal.jsx` (2 c/u) — un módulo por ciclo. |
