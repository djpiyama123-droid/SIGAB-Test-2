...
| Accesibilidad transversal — `<label>` asociado a su control de formulario (Reservas, Órdenes, Equipos, Almacén, Metrología, Copilot) | 🟡 Parcial | v4.0.59 / v4.0.60 / v4.0.61 / v4.0.62 / v4.0.63 / v4.0.64 / v4.0.65 / v4.0.66 / (este ciclo, v4.0.67) | Séptimo barrido de accesibilidad (distinto de los 6 anteriores: `aria-label` de botones de cerrar, `title` de iconos, navegación por teclado en filas/tarjetas, Escape, focus trap, botones sin nombre de Copilot). Hallazgo: `grep -rn "<label" sigab-frontend/src` sin `htmlFor` devolvió 117 líneas en decenas de archivos — cada `<label>Texto</label>` es visualmente correcto pero no está asociado programáticamente a su `<input>/<select>/<textarea>` hermano, así que un lector de pantalla anuncia el campo sin nombre al enfocarlo con Tab. Alcance demasiado grande para un ciclo (regla del proyecto: un módulo a la vez) → v4.0.59 cerró `pages/Reservas.jsx` (`NuevaReservaModal`), v4.0.60 cerró `pages/Ordenes.jsx` (formulario "Nueva Orden de Servicio"), v4.0.61 cerró `components/EquipoForm.jsx`, v4.0.62 cerró `components/OrdenServicioRapidaModal.jsx`, v4.0.63 cerró `components/NuevaOrdenModal.jsx`, v4.0.64 cerró `components/EventoAdversoModal.jsx`, v4.0.65 cerró `pages/Almacen.jsx`, v4.0.66 cerró `pages/Metrologia.jsx` (ver detalle de esas versiones). Ciclo 2026-07-20 (loop-thinkcentre, v4.0.67): `pages/Copilot.jsx` (los 2 paneles de IA, `DiagnosticoPanel` y `VisionPanel`) — de los 6 `<label>` que reportó el grep, 5 son explícitos con control hermano (necesitaban `htmlFor`/`id`) y 1 (el de "Click para seleccionar imagen" en `VisionPanel`, envuelve directamente el `<input type="file">`) ya tenía asociación implícita correcta por anidamiento — no se tocó, agregarle `htmlFor`/`id` habría sido redundante. Se agregó `id`/`htmlFor` a los 5 campos reales: equipo (opcional), marca, modelo y falla reportada de `DiagnosticoPanel`; tipo de documento de `VisionPanel`. Cambio 100% aditivo (solo atributos `id`/`htmlFor`), sin tocar clases, layout, lógica de negocio ni las llamadas a `api.copilotDiagnostico`/`api.copilotVision` — cero riesgo visual o funcional. Verificado con `npm run build` + `npx vitest run` (62 tests) en verde. **Pendiente**: `pages/Trazabilidad.jsx`/`pages/Capacitaciones.jsx` (5 c/u) son ahora los mayores candidatos por volumen; después `components/PreventivoForm.jsx`/`components/OrdenCasillasForm.jsx` (4 c/u), `pages/LandingPage.jsx`/`components/TripleValidationModal.jsx`/`components/ChangePasswordModal.jsx` (3 c/u), `pages/SuperAdmin.jsx`/`pages/ChecklistPage.jsx`/`components/OCRScannerModal.jsx` (2 c/u) — candidato del próximo barrido, un módulo por ciclo. |
