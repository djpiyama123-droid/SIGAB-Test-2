...
| Accesibilidad transversal — `<label>` asociado a su control de formulario (Reservas, Órdenes) | 🟡 Parcial | v4.0.59 / (este ciclo, v4.0.60) | Séptimo barrido de accesibilidad (distinto de los 6 anteriores: `aria-label` de botones de cerrar, `title` de iconos, navegación por teclado en filas/tarjetas, Escape, focus trap, botones sin nombre de Copilot). Hallazgo: `grep -rn "<label" sigab-frontend/src` sin `htmlFor` devolvió 117 líneas en decenas de archivos — cada `<label>Texto</label>` es visualmente correcto pero no está asociado programáticamente a su `<input>/<select>/<textarea>` hermano, así que un lector de pantalla anuncia el campo sin nombre al enfocarlo con Tab. Alcance demasiado grande para un ciclo (regla del proyecto: un módulo a la vez) → v4.0.59 cerró `pages/Reservas.jsx` (`NuevaReservaModal`). Ciclo 2026-07-18 (loop-thinkcentre, v4.0.60): siguiente candidato explícito de la lista pendiente, `pages/Ordenes.jsx` (formulario "Nueva Orden de Servicio", el flujo de mayor tráfico de creación de registros NOM-016 después de Reservas) — los 11 `<label>` (equipo, no. serie, técnico, área, piso, prioridad, formato IMSS, localización completa, falla reportada, hora de inicio, hora de término, tiempo estimado, tiempo real, recibe de conformidad nombre/matrícula) ahora llevan `htmlFor`/`id` (`orden-equipo-nombre`, `orden-equipo-serie`, `orden-tecnico-nombre`, `orden-area`, `orden-piso`, `orden-prioridad`, `orden-tipo-formato`, `orden-localizacion-completa`, `orden-falla-reportada`, `orden-hora-inicio`, `orden-hora-termino`, `orden-tiempo-estimado`, `orden-tiempo-real`, `orden-recibe-conformidad-nombre`, `orden-recibe-matricula`). Los campos Área y Piso alternan entre `<select>` (catálogo cargado) e `<input>` (fallback sin catálogo) según datos disponibles — ambos ramas de cada campo comparten el mismo `id` porque solo una se renderiza a la vez. Cambio 100% aditivo, sin tocar clases/layout/lógica. **Pendiente**: ~100 líneas de `<label>` sin `htmlFor` en el resto de la app (`Copilot.jsx`, `Metrologia.jsx`, `ChecklistPage.jsx`, `Equipos.jsx`, `Almacen.jsx`, `Trazabilidad.jsx`, `Capacitaciones.jsx`, `SuperAdmin.jsx`, `LandingPage.jsx` y varios componentes/modales compartidos como `EventoAdversoModal.jsx`, `NuevaOrdenModal.jsx`, `OrdenServicioRapidaModal.jsx`, `PreventivoForm.jsx`, `TripleValidationModal.jsx`, `ChangePasswordModal.jsx`, `OCRScannerModal.jsx`, `OrdenCasillasForm.jsx`, `EquipoForm.jsx`) — candidato del próximo barrido, un módulo por ciclo. |
