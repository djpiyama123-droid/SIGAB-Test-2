...
| Accesibilidad transversal — `<label>` asociado a su control de formulario (Reservas, Órdenes, Equipos) | 🟡 Parcial | v4.0.59 / v4.0.60 / v4.0.61 / (este ciclo, v4.0.62) | Séptimo barrido de accesibilidad (distinto de los 6 anteriores: `aria-label` de botones de cerrar, `title` de iconos, navegación por teclado en filas/tarjetas, Escape, focus trap, botones sin nombre de Copilot). Hallazgo: `grep -rn "<label" sigab-frontend/src` sin `htmlFor` devolvió 117 líneas en decenas de archivos — cada `<label>Texto</label>` es visualmente correcto pero no está asociado programáticamente a su `<input>/<select>/<textarea>` hermano, así que un lector de pantalla anuncia el campo sin nombre al enfocarlo con Tab. Alcance demasiado grande para un ciclo (regla del proyecto: un módulo a la vez) → v4.0.59 cerró `pages/Reservas.jsx` (`NuevaReservaModal`), v4.0.60 cerró `pages/Ordenes.jsx` (formulario "Nueva Orden de Servicio"), v4.0.61 cerró `components/EquipoForm.jsx` (ver detalle de esas versiones). Ciclo 2026-07-19 (loop-thinkcentre, v4.0.62): `components/OrdenServicioRapidaModal.jsx` (modal de "Nueva Orden de Servicio" que se abre desde el mapa/ficha técnica del equipo — mismo módulo Órdenes, cabecera de la lista pendiente por volumen: 15 `<label>` sin asociar). Se agregó `id`/`htmlFor` a los 11 campos con label+input hermano (tipo de mantenimiento, localización completa, falla reportada, hora de inicio/término, tiempo estimado/real, técnico asignado, recibe de conformidad nombre/matrícula, descripción del trabajo), mismo patrón manual campo-por-campo que Reservas/Ordenes (este archivo no usa un wrapper compartido tipo `Campo` de `EquipoForm.jsx`). El grupo de 3 radios "Prioridad" (Alta/Media/Baja) es distinto: un `<label>` de texto no describe un grupo de opciones, así que se reemplazó el `<div>`+`<label>` contenedor por `<fieldset>`+`<legend>` (con `border-0 p-0 m-0` para no alterar el layout) — patrón WCAG correcto para grupos de radio, primera vez que aparece en el código; los 3 `<label>` que envuelven cada `<input type="radio">` individual ya tenían asociación implícita correcta y no se tocaron. Cambio 100% aditivo (atributos + 1 cambio de elemento semánticamente equivalente), sin tocar clases/layout/lógica de negocio ni el payload enviado al backend. Verificado con `npm run build` + `npx vitest run` (62 tests) en verde. **Pendiente**: `components/NuevaOrdenModal.jsx` (11, mismo módulo Órdenes) y `components/EventoAdversoModal.jsx` (13) son ahora los mayores por volumen; después `pages/Almacen.jsx` (9), `pages/Metrologia.jsx`/`pages/Copilot.jsx` (6 c/u), `pages/Trazabilidad.jsx`/`pages/Capacitaciones.jsx` (5 c/u), `components/PreventivoForm.jsx`/`components/OrdenCasillasForm.jsx` (4 c/u), `pages/LandingPage.jsx`/`components/TripleValidationModal.jsx`/`components/ChangePasswordModal.jsx` (3 c/u), `pages/SuperAdmin.jsx`/`pages/ChecklistPage.jsx`/`components/OCRScannerModal.jsx` (2 c/u) — candidato del próximo barrido, un módulo por ciclo. |
