...
> **Resultado:** Claude Code corriendo desde terminal, con 25+ skills cargadas (11 a nivel proyecto, 14 a nivel usuario) y conectado por SSH al VPS Bluehost (instancia SIGAH) y al servidor Hetzner (cuando exista).

---

## 1. Resumen de lo que va a quedar instalado

### 1.1 Skills a nivel proyecto (`SIGAH/.claude/skills/`)
...
| `mysql` | `sanjay3290/ai-skills` | Consultas SQL **read-only** contra la BD SIGAH/SIGAH en lenguaje natural |
...
- Alias `sigah-bluehost` → VPS donde corre SIGAH hoy (`129.121.100.147`).
...
git clone <URL_DEL_REPO_SIGAH> SIGAH
cd SIGAH
```

> Si ya está en la misma ruta que en el otro equipo (`C:\Users\djpiy\Desktop\Bioingeneria\SIGAH`), simplemente entra al folder.
...
```bash
cd /mnt/c/Users/djpiy/Desktop/Bioingeneria/SIGAH
...
4. Anexa el bloque SIGAH a `~/.ssh/config` y genera la llave `sigah_bluehost_ed25519` si no existe.
...
El script generó `~/.ssh/sigah_bluehost_ed25519` y su `.pub` correspondiente. Hay que pegar el contenido del `.pub` en el VPS:

```powershell
# Imprimir la llave pública (cópiala)
type $env:USERPROFILE\.ssh\sigah_bluehost_ed25519.pub
```
...
Editar `SIGAH/.claude/skills/mysql/connections.json` y reemplazar `REEMPLAZAR_CON_PASSWORD_REAL`:

```json
{
  "name": "sigah-prod-hgr1",
  "host": "127.0.0.1",
  "port": 3306,
  "database": "sigah",
  "user": "sigah_readonly",
...
CREATE USER 'sigah_readonly'@'localhost' IDENTIFIED BY '<password seguro>';
GRANT SELECT ON sigah.* TO 'sigah_readonly'@'localhost';
FLUSH PRIVILEGES;
```
...
> Si el puerto 3306 ya está en uso localmente (porque también tienes MySQL local), usa otro puerto local: `ssh -L 3308:127.0.0.1:3306 sigah-bluehost`, y cambia el `port` en `connections.json` a `3308`.

Probar:

```bash
cd SIGAH/.claude/skills/mysql
python3 scripts/query.py --list
python3 scripts/query.py --db sigah-prod-hgr1 --query "SELECT COUNT(*) FROM equipos"
```
...
- [ ] `ls C:\Users\djpiy\Desktop\Bioingeneria\SIGAH\.claude\skills` muestra 11 carpetas.
- [ ] `ssh sigah-bluehost` conecta sin pedir password.
- [ ] `connections.json` con el password real del usuario read-only.
- [ ] Túnel `ssh -L 3306:127.0.0.1:3306 sigah-bluehost` activo y `python3 scripts/query.py --list` lista la BD.
...
| `Permission denied (publickey)` al hacer `ssh sigah-bluehost` | La llave pública no está en el VPS, o el usuario en `~/.ssh/config` está mal | Verifica `~/.ssh/config` y vuelve a pegar la llave en `~/.ssh/authorized_keys` del VPS. |
| `error connect ECONNREFUSED 127.0.0.1:3306` al probar la skill mysql | No hay túnel SSH activo, o el puerto está ocupado | Levanta el túnel en otra terminal: `ssh -L 3306:127.0.0.1:3306 sigah-bluehost`. Si 3306 está ocupado usa otro puerto local. |
